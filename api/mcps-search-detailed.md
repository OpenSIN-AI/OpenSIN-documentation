# Mcps Search API Endpoint — Detailed Reference

> **Method:** POST | **Resource:** Mcps | **Action:** Search | **Version:** v1

## Endpoint

```
POST /api/v1/mcps/search
```

## Description

Detailed reference for the mcps search endpoint.
...
```bash
curl -X POST https://api.opensin.ai/api/v1/mcps/search \
...
response = requests.post(
    "https://api.opensin.ai/api/v1/mcps/search",
    headers={
...
```go
req, _ := http.NewRequest("POST", "https://api.opensin.ai/api/v1/mcps/search", nil)
...
curl -X POST http://localhost:3000/api/v1/mcps/search

# Test with staging
curl -X POST https://staging-api.opensin.ai/api/v1/mcps/search
```
