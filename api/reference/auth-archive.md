# Auth Archive — API Reference

> **Endpoint:** `PUT /api/v1/auth`/{id} | **Method:** PUT | **Version:** 1.0

## Overview

Detailed API reference for the auth archive endpoint.
...
```bash
curl -X PUT https://api.opensin.ai/api/v1/auth/{id} \
...
curl -X PUT http://localhost:3000/api/v1/auth/{id}

# Test with staging
curl -X PUT https://staging-api.opensin.ai/api/v1/auth/{id}
```
