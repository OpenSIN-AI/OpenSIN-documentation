# Auth Logs API Endpoint — Detailed Reference

> **Method:** GET | **Resource:** Auth | **Action:** Logs | **Version:** v1
...
Detailed reference for the auth logs endpoint.
...
```bash
curl -X GET https://api.opensin.ai/api/v1/auth/logs \
...
response = requests.get(
    "https://api.opensin.ai/api/v1/auth/logs",
    headers={
...
```go
req, _ := http.NewRequest("GET", "https://api.opensin.ai/api/v1/auth/logs", nil)
...
curl -X GET http://localhost:3000/api/v1/auth/logs

# Test with staging
curl -X GET https://staging-api.opensin.ai/api/v1/auth/logs
```
