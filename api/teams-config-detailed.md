# Teams Config API Endpoint — Detailed Reference

> **Method:** GET | **Resource:** Teams | **Action:** Config | **Version:** v1
...
Detailed reference for the teams config endpoint.
...
```bash
curl -X GET https://api.opensin.ai/api/v1/teams/config \
...
response = requests.get(
    "https://api.opensin.ai/api/v1/teams/config",
    headers={
...
```go
req, _ := http.NewRequest("GET", "https://api.opensin.ai/api/v1/teams/config", nil)
...
curl -X GET http://localhost:3000/api/v1/teams/config

# Test with staging
curl -X GET https://staging-api.opensin.ai/api/v1/teams/config
```
