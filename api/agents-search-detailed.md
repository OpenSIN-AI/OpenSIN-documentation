# Agents Search API Endpoint — Detailed Reference

> **Method:** POST | **Resource:** Agents | **Action:** Search | **Version:** v1
...
Detailed reference for the agents search endpoint.
...
```bash
curl -X POST https://api.opensin.ai/api/v1/agents/search \
...
response = requests.post(
    "https://api.opensin.ai/api/v1/agents/search",
    headers={
...
```go
req, _ := http.NewRequest("POST", "https://api.opensin.ai/api/v1/agents/search", nil)
...
curl -X POST http://localhost:3000/api/v1/agents/search

# Test with staging
curl -X POST https://staging-api.opensin.ai/api/v1/agents/search
```
