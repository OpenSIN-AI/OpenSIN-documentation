# OpenSIN Documentation

> **OpenSIN-AI** — The world's most comprehensive autonomous AI agent ecosystem.

## Quick Links

- [Website](https://opensin.ai)
- [Dashboard](https://a2a.delqhi.com)
- [Discord](https://discord.gg/opensin)
- [GitHub](https://github.com/OpenSIN-AI)

## Getting Started

1. [Installation](docs/guide/installation.md)
2. [Quick Start](docs/guide/quick-start.md)
3. [Agent Basics](docs/guide/agent-basics.md)

## Documentation

| Section | Description |
|---------|-------------|
| [Guide](docs/guide/) | User guides and tutorials |
| [Agent Configuration](docs/guide/agent-configuration.md) | 🆕 All agent config files documented |
| [API](docs/api/) | API reference |
| [Architecture](docs/architecture/) | System architecture |
| [Plugins](docs/plugins/) | Plugin documentation |
| [SDK](docs/sdk/) | SDK documentation |
| [Fleet](docs/fleet/) | A2A Agent Fleet |
| [Governance](docs/governance/) | Zeus, Hermes, PR-Watcher |
| [Migrations](docs/migrations/) | Migration guides |
| [Best Practices](docs/best-practices/) | Best practices |
| [Examples](docs/examples/) | Code examples |
| [Tutorials](docs/tutorials/) | Step-by-step tutorials |
| [Bridges](docs/bridges/) | Bridge documentation |

## Ecosystem

| Component | Repository |
|-----------|-----------|
| OpenSIN CLI | [OpenSIN](https://github.com/OpenSIN-AI/OpenSIN) |
| OpenSIN Code | [OpenSIN-Code](https://github.com/OpenSIN-AI/OpenSIN-Code) |
| OpenSIN Backend | [OpenSIN-backend](https://github.com/OpenSIN-AI/OpenSIN-backend) |
| OpenSIN Website | [website-opensin.ai](https://github.com/OpenSIN-AI/website-opensin.ai) |
| My OpenSIN | [website-my.opensin.ai](https://github.com/OpenSIN-AI/website-my.opensin.ai) |
| Upgraded Stack | [upgraded-opencode-stack](https://github.com/Delqhi/upgraded-opencode-stack) |
| Canonical Flow Runtime | [SIN-InkogniFlow](https://github.com/OpenSIN-AI/SIN-InkogniFlow) |

## Development

```bash
bun install
bun run docs:dev
bun run docs:build
bun test
```

## License

MIT
