# Sin Pinterest Agent — Troubleshooting Guide

> **Agent:** sin-pinterest-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-pinterest-agent`
2. Restart agent: `opencode run --agent "sin-pinterest-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-pinterest-agent.log`
...
1. Collect logs: `journalctl -u sin-pinterest-agent --no-pager > /tmp/sin-pinterest-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-pinterest-agent/config.yaml > /tmp/sin-pinterest-agent-config.txt`
