# Sin Finance Agent — Troubleshooting Guide

> **Agent:** sin-finance-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-finance-agent`
2. Restart agent: `opencode run --agent "sin-finance-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-finance-agent.log`
...
1. Collect logs: `journalctl -u sin-finance-agent --no-pager > /tmp/sin-finance-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-finance-agent/config.yaml > /tmp/sin-finance-agent-config.txt`
