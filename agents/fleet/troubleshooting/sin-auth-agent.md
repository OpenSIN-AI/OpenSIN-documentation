# Sin Auth Agent — Troubleshooting Guide

> **Agent:** sin-auth-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-auth-agent`
2. Restart agent: `opencode run --agent "sin-auth-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-auth-agent.log`
...
1. Collect logs: `journalctl -u sin-auth-agent --no-pager > /tmp/sin-auth-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-auth-agent/config.yaml > /tmp/sin-auth-agent-config.txt`
