# Sin Survey Agent — Troubleshooting Guide

> **Agent:** sin-survey-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-survey-agent`
2. Restart agent: `opencode run --agent "sin-survey-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-survey-agent.log`
...
1. Collect logs: `journalctl -u sin-survey-agent --no-pager > /tmp/sin-survey-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-survey-agent/config.yaml > /tmp/sin-survey-agent-config.txt`
