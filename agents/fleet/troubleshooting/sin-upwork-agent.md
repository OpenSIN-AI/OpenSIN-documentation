# Sin Upwork Agent — Troubleshooting Guide

> **Agent:** sin-upwork-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-upwork-agent`
2. Restart agent: `opencode run --agent "sin-upwork-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-upwork-agent.log`
...
1. Collect logs: `journalctl -u sin-upwork-agent --no-pager > /tmp/sin-upwork-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-upwork-agent/config.yaml > /tmp/sin-upwork-agent-config.txt`
