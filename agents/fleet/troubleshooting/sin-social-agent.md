# Sin Social Agent — Troubleshooting Guide

> **Agent:** sin-social-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-social-agent`
2. Restart agent: `opencode run --agent "sin-social-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-social-agent.log`
...
1. Collect logs: `journalctl -u sin-social-agent --no-pager > /tmp/sin-social-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-social-agent/config.yaml > /tmp/sin-social-agent-config.txt`
