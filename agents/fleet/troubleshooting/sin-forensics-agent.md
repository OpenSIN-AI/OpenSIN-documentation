# Sin Forensics Agent — Troubleshooting Guide

> **Agent:** sin-forensics-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-forensics-agent`
2. Restart agent: `opencode run --agent "sin-forensics-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-forensics-agent.log`
...
1. Collect logs: `journalctl -u sin-forensics-agent --no-pager > /tmp/sin-forensics-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-forensics-agent/config.yaml > /tmp/sin-forensics-agent-config.txt`
