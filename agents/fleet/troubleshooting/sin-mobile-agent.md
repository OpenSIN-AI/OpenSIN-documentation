# Sin Mobile Agent — Troubleshooting Guide

> **Agent:** sin-mobile-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-mobile-agent`
2. Restart agent: `opencode run --agent "sin-mobile-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-mobile-agent.log`
...
1. Collect logs: `journalctl -u sin-mobile-agent --no-pager > /tmp/sin-mobile-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-mobile-agent/config.yaml > /tmp/sin-mobile-agent-config.txt`
