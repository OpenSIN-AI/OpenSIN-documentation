# Sin Linkedin Agent — Troubleshooting Guide

> **Agent:** sin-linkedin-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-linkedin-agent`
2. Restart agent: `opencode run --agent "sin-linkedin-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-linkedin-agent.log`
...
1. Collect logs: `journalctl -u sin-linkedin-agent --no-pager > /tmp/sin-linkedin-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-linkedin-agent/config.yaml > /tmp/sin-linkedin-agent-config.txt`
