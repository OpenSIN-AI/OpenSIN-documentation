# Sin Backend Agent — Configuration Reference

> **Agent:** sin-backend-agent | **Team:** Backend | **Status:** ✅ Active

## Agent Card

```json
{
  "name": "sin-backend-agent",
  "version": "1.0.0",
  "description": "Autonomous A2A agent for sin backend agent",
  "team": "backend",
...
|----------|----------|---------|-------------|
| `SIN_BACKEND_AGENT_API_KEY` | No | — | API key for external services |
| `SIN_BACKEND_AGENT_ENDPOINT` | No | — | Custom endpoint URL |
| `SIN_BACKEND_AGENT_TIMEOUT` | No | 30 | Request timeout in seconds |
| `SIN_BACKEND_AGENT_RETRIES` | No | 3 | Number of retries on failure |
| `SIN_BACKEND_AGENT_LOG_LEVEL` | No | info | Logging level |
