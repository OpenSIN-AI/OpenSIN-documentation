# Sin Youtube Agent — Troubleshooting Guide

> **Agent:** sin-youtube-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-youtube-agent`
2. Restart agent: `opencode run --agent "sin-youtube-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-youtube-agent.log`
...
1. Collect logs: `journalctl -u sin-youtube-agent --no-pager > /tmp/sin-youtube-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-youtube-agent/config.yaml > /tmp/sin-youtube-agent-config.txt`
