# Sin Dns Agent — Deployment Guide

> **Agent:** sin-dns-agent | **Category:** A2A Agent Deployment | **Version:** 1.0 | **Status:** Active

## Overview

This guide provides step-by-step deployment instructions for the Sin Dns Agent.
...
```bash
git clone git@github.com:OpenSIN-AI/sin-dns-agent.git
cd sin-dns-agent
```
...
curl -X POST http://92.5.60.87:8006/rest/v1/agent_registry \
  ...
  -d '{"name": "sin-dns-agent", "status": "active", "model": "openrouter/qwen/qwen3.6-plus:free"}'
```
...
huggingface-cli repo create sin-dns-agent --type space --space_sdk docker
git push https://huggingface.co/spaces/delqhi/sin-dns-agent
```
...
curl http://92.5.60.87:8006/rest/v1/agent_registry?name=eq.sin-dns-agent \
  -H "apikey: $SUPABASE_KEY"
```
...
- [Agent Reference Card](../reference-cards/sin-dns-agent.md)
- [Integration Guide](../../integrations/agents/sin-dns-agent.md)
- [Team Playbook](../team-playbooks/)
- [Agent Configuration](../fleet/configs/sin-dns-agent.md)
