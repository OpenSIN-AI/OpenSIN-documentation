# Sin Backend Agent — Deployment Guide

> **Agent:** sin-backend-agent | **Category:** A2A Agent Deployment | **Version:** 1.0 | **Status:** Active

## Overview

This guide provides step-by-step deployment instructions for the Sin Backend Agent.
...
```bash
git clone git@github.com:OpenSIN-AI/sin-backend-agent.git
cd sin-backend-agent
```
...
curl -X POST http://92.5.60.87:8006/rest/v1/agent_registry \
  ...
  -d '{"name": "sin-backend-agent", "status": "active", "model": "openrouter/qwen/qwen3.6-plus:free"}'
```
...
huggingface-cli repo create sin-backend-agent --type space --space_sdk docker
git push https://huggingface.co/spaces/delqhi/sin-backend-agent
```
...
curl http://92.5.60.87:8006/rest/v1/agent_registry?name=eq.sin-backend-agent \
  -H "apikey: $SUPABASE_KEY"
```
...
- [Agent Reference Card](../reference-cards/sin-backend-agent.md)
- [Integration Guide](../../integrations/agents/sin-backend-agent.md)
- [Team Playbook](../team-playbooks/)
- [Agent Configuration](../fleet/configs/sin-backend-agent.md)
