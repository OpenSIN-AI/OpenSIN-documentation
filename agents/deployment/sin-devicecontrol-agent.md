# Sin Devicecontrol Agent — Deployment Guide

> **Agent:** sin-devicecontrol-agent | **Category:** A2A Agent Deployment | **Version:** 1.0 | **Status:** Active

## Overview

This guide provides step-by-step deployment instructions for the Sin Devicecontrol Agent.
...
```bash
git clone git@github.com:OpenSIN-AI/sin-devicecontrol-agent.git
cd sin-devicecontrol-agent
```
...
curl -X POST http://92.5.60.87:8006/rest/v1/agent_registry \
  ...
  -d '{"name": "sin-devicecontrol-agent", "status": "active", "model": "openrouter/qwen/qwen3.6-plus:free"}'
```
...
huggingface-cli repo create sin-devicecontrol-agent --type space --space_sdk docker
git push https://huggingface.co/spaces/delqhi/sin-devicecontrol-agent
```
...
curl http://92.5.60.87:8006/rest/v1/agent_registry?name=eq.sin-devicecontrol-agent \
  -H "apikey: $SUPABASE_KEY"
```
...
- [Agent Reference Card](../reference-cards/sin-devicecontrol-agent.md)
- [Integration Guide](../../integrations/agents/sin-devicecontrol-agent.md)
- [Team Playbook](../team-playbooks/)
- [Agent Configuration](../fleet/configs/sin-devicecontrol-agent.md)
