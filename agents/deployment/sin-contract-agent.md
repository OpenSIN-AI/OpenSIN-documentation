# Sin Contract Agent — Deployment Guide

> **Agent:** sin-contract-agent | **Category:** A2A Agent Deployment | **Version:** 1.0 | **Status:** Active

## Overview

This guide provides step-by-step deployment instructions for the Sin Contract Agent.
...
```bash
git clone git@github.com:OpenSIN-AI/sin-contract-agent.git
cd sin-contract-agent
```
...
curl -X POST http://92.5.60.87:8006/rest/v1/agent_registry \
  ...
  -d '{"name": "sin-contract-agent", "status": "active", "model": "openrouter/qwen/qwen3.6-plus:free"}'
```
...
huggingface-cli repo create sin-contract-agent --type space --space_sdk docker
git push https://huggingface.co/spaces/delqhi/sin-contract-agent
```
...
curl http://92.5.60.87:8006/rest/v1/agent_registry?name=eq.sin-contract-agent \
  -H "apikey: $SUPABASE_KEY"
```
...
- [Agent Reference Card](../reference-cards/sin-contract-agent.md)
- [Integration Guide](../../integrations/agents/sin-contract-agent.md)
- [Team Playbook](../team-playbooks/)
- [Agent Configuration](../fleet/configs/sin-contract-agent.md)
