# Sin Facebook Agent — Deployment Guide

> **Agent:** sin-facebook-agent | **Category:** A2A Agent Deployment | **Version:** 1.0 | **Status:** Active

## Overview

This guide provides step-by-step deployment instructions for the Sin Facebook Agent.
...
```bash
git clone git@github.com:OpenSIN-AI/sin-facebook-agent.git
cd sin-facebook-agent
```
...
curl -X POST http://92.5.60.87:8006/rest/v1/agent_registry \
  ...
  -d '{"name": "sin-facebook-agent", "status": "active", "model": "openrouter/qwen/qwen3.6-plus:free"}'
```
...
huggingface-cli repo create sin-facebook-agent --type space --space_sdk docker
git push https://huggingface.co/spaces/delqhi/sin-facebook-agent
```
...
curl http://92.5.60.87:8006/rest/v1/agent_registry?name=eq.sin-facebook-agent \
  -H "apikey: $SUPABASE_KEY"
```
...
- [Agent Reference Card](../reference-cards/sin-facebook-agent.md)
- [Integration Guide](../../integrations/agents/sin-facebook-agent.md)
- [Team Playbook](../team-playbooks/)
- [Agent Configuration](../fleet/configs/sin-facebook-agent.md)
