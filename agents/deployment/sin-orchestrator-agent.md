# Sin Orchestrator Agent — Deployment Guide

> **Agent:** sin-orchestrator-agent | **Category:** A2A Agent Deployment | **Version:** 1.0 | **Status:** Active

## Overview

This guide provides step-by-step deployment instructions for the Sin Orchestrator Agent.
...
```bash
git clone git@github.com:OpenSIN-AI/sin-orchestrator-agent.git
cd sin-orchestrator-agent
```
...
curl -X POST http://92.5.60.87:8006/rest/v1/agent_registry \
  ...
  -d '{"name": "sin-orchestrator-agent", "status": "active", "model": "openrouter/qwen/qwen3.6-plus:free"}'
```
...
huggingface-cli repo create sin-orchestrator-agent --type space --space_sdk docker
git push https://huggingface.co/spaces/delqhi/sin-orchestrator-agent
```
...
curl http://92.5.60.87:8006/rest/v1/agent_registry?name=eq.sin-orchestrator-agent \
  -H "apikey: $SUPABASE_KEY"
```
...
- [Agent Reference Card](../reference-cards/sin-orchestrator-agent.md)
- [Integration Guide](../../integrations/agents/sin-orchestrator-agent.md)
- [Team Playbook](../team-playbooks/)
- [Agent Configuration](../fleet/configs/sin-orchestrator-agent.md)
