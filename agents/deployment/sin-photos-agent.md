# Sin Photos Agent — Deployment Guide

> **Agent:** sin-photos-agent | **Category:** A2A Agent Deployment | **Version:** 1.0 | **Status:** Active

## Overview

This guide provides step-by-step deployment instructions for the Sin Photos Agent.
...
```bash
git clone git@github.com:OpenSIN-AI/sin-photos-agent.git
cd sin-photos-agent
```
...
curl -X POST http://92.5.60.87:8006/rest/v1/agent_registry \
  ...
  -d '{"name": "sin-photos-agent", "status": "active", "model": "openrouter/qwen/qwen3.6-plus:free"}'
```
...
huggingface-cli repo create sin-photos-agent --type space --space_sdk docker
git push https://huggingface.co/spaces/delqhi/sin-photos-agent
```
...
curl http://92.5.60.87:8006/rest/v1/agent_registry?name=eq.sin-photos-agent \
  -H "apikey: $SUPABASE_KEY"
```
...
- [Agent Reference Card](../reference-cards/sin-photos-agent.md)
- [Integration Guide](../../integrations/agents/sin-photos-agent.md)
- [Team Playbook](../team-playbooks/)
- [Agent Configuration](../fleet/configs/sin-photos-agent.md)
