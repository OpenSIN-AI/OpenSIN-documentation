# Sin Prolific Agent — Deployment Guide

> **Agent:** sin-prolific-agent | **Category:** A2A Agent Deployment | **Version:** 1.0 | **Status:** Active

## Overview

This guide provides step-by-step deployment instructions for the Sin Prolific Agent.
...
```bash
git clone git@github.com:OpenSIN-AI/sin-prolific-agent.git
cd sin-prolific-agent
```
...
curl -X POST http://92.5.60.87:8006/rest/v1/agent_registry \
  ...
  -d '{"name": "sin-prolific-agent", "status": "active", "model": "openrouter/qwen/qwen3.6-plus:free"}'
```
...
huggingface-cli repo create sin-prolific-agent --type space --space_sdk docker
git push https://huggingface.co/spaces/delqhi/sin-prolific-agent
```
...
curl http://92.5.60.87:8006/rest/v1/agent_registry?name=eq.sin-prolific-agent \
  -H "apikey: $SUPABASE_KEY"
```
...
- [Agent Reference Card](../reference-cards/sin-prolific-agent.md)
- [Integration Guide](../../integrations/agents/sin-prolific-agent.md)
- [Team Playbook](../team-playbooks/)
- [Agent Configuration](../fleet/configs/sin-prolific-agent.md)
