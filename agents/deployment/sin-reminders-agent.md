# Sin Reminders Agent — Deployment Guide

> **Agent:** sin-reminders-agent | **Category:** A2A Agent Deployment | **Version:** 1.0 | **Status:** Active

## Overview

This guide provides step-by-step deployment instructions for the Sin Reminders Agent.
...
```bash
git clone git@github.com:OpenSIN-AI/sin-reminders-agent.git
cd sin-reminders-agent
```
...
curl -X POST http://92.5.60.87:8006/rest/v1/agent_registry \
  ...
  -d '{"name": "sin-reminders-agent", "status": "active", "model": "openrouter/qwen/qwen3.6-plus:free"}'
```
...
huggingface-cli repo create sin-reminders-agent --type space --space_sdk docker
git push https://huggingface.co/spaces/delqhi/sin-reminders-agent
```
...
curl http://92.5.60.87:8006/rest/v1/agent_registry?name=eq.sin-reminders-agent \
  -H "apikey: $SUPABASE_KEY"
```
...
- [Agent Reference Card](../reference-cards/sin-reminders-agent.md)
- [Integration Guide](../../integrations/agents/sin-reminders-agent.md)
- [Team Playbook](../team-playbooks/)
- [Agent Configuration](../fleet/configs/sin-reminders-agent.md)
