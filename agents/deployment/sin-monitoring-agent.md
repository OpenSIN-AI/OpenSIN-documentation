# Sin Monitoring Agent — Deployment Guide

> **Agent:** sin-monitoring-agent | **Category:** A2A Agent Deployment | **Version:** 1.0 | **Status:** Active

## Overview

This guide provides step-by-step deployment instructions for the Sin Monitoring Agent.
...
```bash
git clone git@github.com:OpenSIN-AI/sin-monitoring-agent.git
cd sin-monitoring-agent
```
...
curl -X POST http://92.5.60.87:8006/rest/v1/agent_registry \
  ...
  -d '{"name": "sin-monitoring-agent", "status": "active", "model": "openrouter/qwen/qwen3.6-plus:free"}'
```
...
huggingface-cli repo create sin-monitoring-agent --type space --space_sdk docker
git push https://huggingface.co/spaces/delqhi/sin-monitoring-agent
```
...
curl http://92.5.60.87:8006/rest/v1/agent_registry?name=eq.sin-monitoring-agent \
  -H "apikey: $SUPABASE_KEY"
```
...
- [Agent Reference Card](../reference-cards/sin-monitoring-agent.md)
- [Integration Guide](../../integrations/agents/sin-monitoring-agent.md)
- [Team Playbook](../team-playbooks/)
- [Agent Configuration](../fleet/configs/sin-monitoring-agent.md)
