# Sin Security Agent — Deployment Guide

> **Agent:** sin-security-agent | **Category:** A2A Agent Deployment | **Version:** 1.0 | **Status:** Active

## Overview

This guide provides step-by-step deployment instructions for the Sin Security Agent.
...
```bash
git clone git@github.com:OpenSIN-AI/sin-security-agent.git
cd sin-security-agent
```
...
curl -X POST http://92.5.60.87:8006/rest/v1/agent_registry \
  ...
  -d '{"name": "sin-security-agent", "status": "active", "model": "openrouter/qwen/qwen3.6-plus:free"}'
```
...
huggingface-cli repo create sin-security-agent --type space --space_sdk docker
git push https://huggingface.co/spaces/delqhi/sin-security-agent
```
...
curl http://92.5.60.87:8006/rest/v1/agent_registry?name=eq.sin-security-agent \
  -H "apikey: $SUPABASE_KEY"
```
...
- [Agent Reference Card](../reference-cards/sin-security-agent.md)
- [Integration Guide](../../integrations/agents/sin-security-agent.md)
- [Team Playbook](../team-playbooks/)
- [Agent Configuration](../fleet/configs/sin-security-agent.md)
