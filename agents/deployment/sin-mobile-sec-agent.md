# Sin Mobile Sec Agent — Deployment Guide

> **Agent:** sin-mobile-sec-agent | **Category:** A2A Agent Deployment | **Version:** 1.0 | **Status:** Active

## Overview

This guide provides step-by-step deployment instructions for the Sin Mobile Sec Agent.
...
```bash
git clone git@github.com:OpenSIN-AI/sin-mobile-sec-agent.git
cd sin-mobile-sec-agent
```
...
curl -X POST http://92.5.60.87:8006/rest/v1/agent_registry \
  ...
  -d '{"name": "sin-mobile-sec-agent", "status": "active", "model": "openrouter/qwen/qwen3.6-plus:free"}'
```
...
huggingface-cli repo create sin-mobile-sec-agent --type space --space_sdk docker
git push https://huggingface.co/spaces/delqhi/sin-mobile-sec-agent
```
...
curl http://92.5.60.87:8006/rest/v1/agent_registry?name=eq.sin-mobile-sec-agent \
  -H "apikey: $SUPABASE_KEY"
```
...
- [Agent Reference Card](../reference-cards/sin-mobile-sec-agent.md)
- [Integration Guide](../../integrations/agents/sin-mobile-sec-agent.md)
- [Team Playbook](../team-playbooks/)
- [Agent Configuration](../fleet/configs/sin-mobile-sec-agent.md)
