# Sin Datascience Agent — Deployment Guide

> **Agent:** sin-datascience-agent | **Category:** A2A Agent Deployment | **Version:** 1.0 | **Status:** Active

## Overview

This guide provides step-by-step deployment instructions for the Sin Datascience Agent.
...
```bash
git clone git@github.com:OpenSIN-AI/sin-datascience-agent.git
cd sin-datascience-agent
```
...
curl -X POST http://92.5.60.87:8006/rest/v1/agent_registry \
  ...
  -d '{"name": "sin-datascience-agent", "status": "active", "model": "openrouter/qwen/qwen3.6-plus:free"}'
```
...
huggingface-cli repo create sin-datascience-agent --type space --space_sdk docker
git push https://huggingface.co/spaces/delqhi/sin-datascience-agent
```
...
curl http://92.5.60.87:8006/rest/v1/agent_registry?name=eq.sin-datascience-agent \
  -H "apikey: $SUPABASE_KEY"
```
...
- [Agent Reference Card](../reference-cards/sin-datascience-agent.md)
- [Integration Guide](../../integrations/agents/sin-datascience-agent.md)
- [Team Playbook](../team-playbooks/)
- [Agent Configuration](../fleet/configs/sin-datascience-agent.md)
