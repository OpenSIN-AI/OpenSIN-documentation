# Sin Pinterest Agent — Deployment Guide

> **Agent:** sin-pinterest-agent | **Category:** A2A Agent Deployment | **Version:** 1.0 | **Status:** Active

## Overview

This guide provides step-by-step deployment instructions for the Sin Pinterest Agent.
...
```bash
git clone git@github.com:OpenSIN-AI/sin-pinterest-agent.git
cd sin-pinterest-agent
```
...
curl -X POST http://92.5.60.87:8006/rest/v1/agent_registry \
  ...
  -d '{"name": "sin-pinterest-agent", "status": "active", "model": "openrouter/qwen/qwen3.6-plus:free"}'
```
...
huggingface-cli repo create sin-pinterest-agent --type space --space_sdk docker
git push https://huggingface.co/spaces/delqhi/sin-pinterest-agent
```
...
curl http://92.5.60.87:8006/rest/v1/agent_registry?name=eq.sin-pinterest-agent \
  -H "apikey: $SUPABASE_KEY"
```
...
- [Agent Reference Card](../reference-cards/sin-pinterest-agent.md)
- [Integration Guide](../../integrations/agents/sin-pinterest-agent.md)
- [Team Playbook](../team-playbooks/)
- [Agent Configuration](../fleet/configs/sin-pinterest-agent.md)
