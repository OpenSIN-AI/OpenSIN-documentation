# Sin Calendar Agent — Deployment Guide

> **Agent:** sin-calendar-agent | **Category:** A2A Agent Deployment | **Version:** 1.0 | **Status:** Active

## Overview

This guide provides step-by-step deployment instructions for the Sin Calendar Agent.
...
```bash
git clone git@github.com:OpenSIN-AI/sin-calendar-agent.git
cd sin-calendar-agent
```
...
curl -X POST http://92.5.60.87:8006/rest/v1/agent_registry \
  ...
  -d '{"name": "sin-calendar-agent", "status": "active", "model": "openrouter/qwen/qwen3.6-plus:free"}'
```
...
huggingface-cli repo create sin-calendar-agent --type space --space_sdk docker
git push https://huggingface.co/spaces/delqhi/sin-calendar-agent
```
...
curl http://92.5.60.87:8006/rest/v1/agent_registry?name=eq.sin-calendar-agent \
  -H "apikey: $SUPABASE_KEY"
```
...
- [Agent Reference Card](../reference-cards/sin-calendar-agent.md)
- [Integration Guide](../../integrations/agents/sin-calendar-agent.md)
- [Team Playbook](../team-playbooks/)
- [Agent Configuration](../fleet/configs/sin-calendar-agent.md)
