# Sin Facetime Agent — Deployment Guide

> **Agent:** sin-facetime-agent | **Category:** A2A Agent Deployment | **Version:** 1.0 | **Status:** Active

## Overview

This guide provides step-by-step deployment instructions for the Sin Facetime Agent.
...
```bash
git clone git@github.com:OpenSIN-AI/sin-facetime-agent.git
cd sin-facetime-agent
```
...
curl -X POST http://92.5.60.87:8006/rest/v1/agent_registry \
  ...
  -d '{"name": "sin-facetime-agent", "status": "active", "model": "openrouter/qwen/qwen3.6-plus:free"}'
```
...
huggingface-cli repo create sin-facetime-agent --type space --space_sdk docker
git push https://huggingface.co/spaces/delqhi/sin-facetime-agent
```
...
curl http://92.5.60.87:8006/rest/v1/agent_registry?name=eq.sin-facetime-agent \
  -H "apikey: $SUPABASE_KEY"
```
...
- [Agent Reference Card](../reference-cards/sin-facetime-agent.md)
- [Integration Guide](../../integrations/agents/sin-facetime-agent.md)
- [Team Playbook](../team-playbooks/)
- [Agent Configuration](../fleet/configs/sin-facetime-agent.md)
