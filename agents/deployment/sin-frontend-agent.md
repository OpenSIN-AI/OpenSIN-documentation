# Sin Frontend Agent — Deployment Guide

> **Agent:** sin-frontend-agent | **Category:** A2A Agent Deployment | **Version:** 1.0 | **Status:** Active

## Overview

This guide provides step-by-step deployment instructions for the Sin Frontend Agent.
...
```bash
git clone git@github.com:OpenSIN-AI/sin-frontend-agent.git
cd sin-frontend-agent
```
...
curl -X POST http://92.5.60.87:8006/rest/v1/agent_registry \
  ...
  -d '{"name": "sin-frontend-agent", "status": "active", "model": "openrouter/qwen/qwen3.6-plus:free"}'
```
...
huggingface-cli repo create sin-frontend-agent --type space --space_sdk docker
git push https://huggingface.co/spaces/delqhi/sin-frontend-agent
```
...
curl http://92.5.60.87:8006/rest/v1/agent_registry?name=eq.sin-frontend-agent \
  -H "apikey: $SUPABASE_KEY"
```
...
- [Agent Reference Card](../reference-cards/sin-frontend-agent.md)
- [Integration Guide](../../integrations/agents/sin-frontend-agent.md)
- [Team Playbook](../team-playbooks/)
- [Agent Configuration](../fleet/configs/sin-frontend-agent.md)
