# Sin Prolific Agent — Troubleshooting Guide

> **Agent:** sin-prolific-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-prolific-agent`
2. Restart agent: `opencode run --agent "sin-prolific-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-prolific-agent.log`
...
1. Collect logs: `journalctl -u sin-prolific-agent --no-pager > /tmp/sin-prolific-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-prolific-agent/config.yaml > /tmp/sin-prolific-agent-config.txt`
