# Sin Devto Agent — Troubleshooting Guide

> **Agent:** sin-devto-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-devto-agent`
2. Restart agent: `opencode run --agent "sin-devto-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-devto-agent.log`
...
1. Collect logs: `journalctl -u sin-devto-agent --no-pager > /tmp/sin-devto-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-devto-agent/config.yaml > /tmp/sin-devto-agent-config.txt`
