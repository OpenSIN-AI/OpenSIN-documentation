# Sin Cloud Agent — Troubleshooting Guide

> **Agent:** sin-cloud-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-cloud-agent`
2. Restart agent: `opencode run --agent "sin-cloud-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-cloud-agent.log`
...
1. Collect logs: `journalctl -u sin-cloud-agent --no-pager > /tmp/sin-cloud-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-cloud-agent/config.yaml > /tmp/sin-cloud-agent-config.txt`
