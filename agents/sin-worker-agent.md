# Sin Worker Agent — Troubleshooting Guide

> **Agent:** sin-worker-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-worker-agent`
2. Restart agent: `opencode run --agent "sin-worker-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-worker-agent.log`
...
1. Collect logs: `journalctl -u sin-worker-agent --no-pager > /tmp/sin-worker-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-worker-agent/config.yaml > /tmp/sin-worker-agent-config.txt`
