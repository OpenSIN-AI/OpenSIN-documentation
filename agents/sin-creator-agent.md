# Sin Creator Agent — Troubleshooting Guide

> **Agent:** sin-creator-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-creator-agent`
2. Restart agent: `opencode run --agent "sin-creator-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-creator-agent.log`
...
1. Collect logs: `journalctl -u sin-creator-agent --no-pager > /tmp/sin-creator-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-creator-agent/config.yaml > /tmp/sin-creator-agent-config.txt`
