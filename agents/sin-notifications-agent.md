# Sin Notifications Agent — Troubleshooting Guide

> **Agent:** sin-notifications-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-notifications-agent`
2. Restart agent: `opencode run --agent "sin-notifications-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-notifications-agent.log`
...
1. Collect logs: `journalctl -u sin-notifications-agent --no-pager > /tmp/sin-notifications-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-notifications-agent/config.yaml > /tmp/sin-notifications-agent-config.txt`
