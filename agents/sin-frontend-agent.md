# Sin Frontend Agent — Troubleshooting Guide

> **Agent:** sin-frontend-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-frontend-agent`
2. Restart agent: `opencode run --agent "sin-frontend-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-frontend-agent.log`
...
1. Collect logs: `journalctl -u sin-frontend-agent --no-pager > /tmp/sin-frontend-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-frontend-agent/config.yaml > /tmp/sin-frontend-agent-config.txt`
