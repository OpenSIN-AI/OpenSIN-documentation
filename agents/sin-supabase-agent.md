# Sin Supabase Agent — Troubleshooting Guide

> **Agent:** sin-supabase-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-supabase-agent`
2. Restart agent: `opencode run --agent "sin-supabase-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-supabase-agent.log`
...
1. Collect logs: `journalctl -u sin-supabase-agent --no-pager > /tmp/sin-supabase-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-supabase-agent/config.yaml > /tmp/sin-supabase-agent-config.txt`
