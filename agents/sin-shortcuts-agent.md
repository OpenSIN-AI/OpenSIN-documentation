# Sin Shortcuts Agent — Troubleshooting Guide

> **Agent:** sin-shortcuts-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-shortcuts-agent`
2. Restart agent: `opencode run --agent "sin-shortcuts-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-shortcuts-agent.log`
...
1. Collect logs: `journalctl -u sin-shortcuts-agent --no-pager > /tmp/sin-shortcuts-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-shortcuts-agent/config.yaml > /tmp/sin-shortcuts-agent-config.txt`
