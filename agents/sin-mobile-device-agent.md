# Sin Mobile Device Agent — Troubleshooting Guide

> **Agent:** sin-mobile-device-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-mobile-device-agent`
2. Restart agent: `opencode run --agent "sin-mobile-device-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-mobile-device-agent.log`
...
1. Collect logs: `journalctl -u sin-mobile-device-agent --no-pager > /tmp/sin-mobile-device-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-mobile-device-agent/config.yaml > /tmp/sin-mobile-device-agent-config.txt`
