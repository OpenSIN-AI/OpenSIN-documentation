# Github Release Fleet Sync Notifier — JSON Configuration

> **Workflow:** github-release-fleet-sync-notifier | **Category:** n8n Workflow Config | **Status:** Active

## Workflow JSON

```json
{
  "name": "Github Release Fleet Sync Notifier",
  "nodes": [
    {
      "parameters": {
        "rule": {
    ...
    },
    {
      "parameters": {
        ...
        "sendBody": true,
        "specifyBody": "json",
        "jsonBody": "={"chat_id": "{{ $env.TELEGRAM_CHAT_ID }}", "text": "📚 Github Release Fleet Sync Notifier\nTime: {{ $now.format('yyyy-MM-dd HH:mm') }} "}",
        "options": {}
      },
