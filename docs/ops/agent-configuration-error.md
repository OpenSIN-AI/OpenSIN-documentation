# Agent Configuration Error — Troubleshooting Guide

> **Category:** Troubleshooting | **Severity:** High | **Version:** 1.0 | **Status:** Active

## Symptoms

- Agent Configuration Error errors in logs
...
# Check recent logs
journalctl -u agent -n 100 --no-pager
docker logs agent --tail 100
```
...
docker exec agent cat /app/config.json
```
...
docker exec agent rm -rf /tmp/*
```

### Solution 3: Update Configuration
```bash
# Edit configuration
nano /etc/agent/config.yaml
...
docker update --memory=4g agent
docker update --cpus=2 agent
```
...
docker stop agent
docker rm agent
docker run -d --name agent previous-image
```
