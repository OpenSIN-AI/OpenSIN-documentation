# Oci Vm Volume Detached — Troubleshooting Guide

> **Category:** Troubleshooting | **Severity:** High | **Version:** 1.0 | **Status:** Active

## Symptoms

- Oci Vm Volume Detached errors in logs
...
# Check recent logs
journalctl -u oci -n 100 --no-pager
...
cat /etc/oci/config.yaml
docker exec oci cat /app/config.json
```
...
```bash
rm -rf /var/cache/oci/*
docker exec oci rm -rf /tmp/*
```

### Solution 3: Update Configuration
```bash
# Edit configuration
nano /etc/oci/config.yaml
...
docker update --memory=4g oci
docker update --cpus=2 oci
```
...
docker stop oci
docker rm oci
docker run -d --name oci previous-image
```
