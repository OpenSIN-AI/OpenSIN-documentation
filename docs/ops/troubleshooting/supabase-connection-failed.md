# Supabase Connection Failed — Troubleshooting Guide

> **Category:** Troubleshooting | **Severity:** High | **Version:** 1.0 | **Status:** Active

## Symptoms

- Supabase Connection Failed errors in logs
...
# Check recent logs
journalctl -u supabase -n 100 --no-pager
docker logs supabase --tail 100
```
...
# Check network
ping -c 3 supabase.local
```

### Step 3: Check Configuration
```bash
# Verify configuration
cat /etc/supabase/config.yaml
docker exec supabase cat /app/config.json
```
...
# or
docker restart supabase
```

### Solution 2: Clear Cache
```bash
rm -rf /var/cache/supabase/*
docker exec supabase rm -rf /tmp/*
```

### Solution 3: Update Configuration
```bash
# Edit configuration
nano /etc/supabase/config.yaml
...
docker update --memory=4g supabase
docker update --cpus=2 supabase
```

### Solution 5: Rollback
```bash
# Rollback to previous version
docker stop supabase
docker rm supabase
docker run -d --name supabase previous-image
```
