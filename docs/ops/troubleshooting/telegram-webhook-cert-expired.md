# Telegram Webhook Cert Expired — Troubleshooting Guide

> **Category:** Troubleshooting | **Severity:** High | **Version:** 1.0 | **Status:** Active

## Symptoms

- Telegram Webhook Cert Expired errors in logs
...
systemctl status telegram
docker ps | grep telegram

# Check recent logs
journalctl -u telegram -n 100 --no-pager
docker logs telegram --tail 100
```
...
# Check network
ping -c 3 telegram.local
```

### Step 3: Check Configuration
```bash
# Verify configuration
cat /etc/telegram/config.yaml
docker exec telegram cat /app/config.json
```
...
```bash
rm -rf /var/cache/telegram/*
docker exec telegram rm -rf /tmp/*
```

### Solution 3: Update Configuration
```bash
# Edit configuration
nano /etc/telegram/config.yaml
...
docker update --memory=4g telegram
docker update --cpus=2 telegram
```
...
docker rm telegram
docker run -d --name telegram previous-image
```
