...
Ein Agent darf **NIEMALS** versuchen, tiefe Architektur- oder Systemfehler allein und isoliert in einem "Silo" zu reparieren. Stattdessen wird jeder Fehler, Absturz oder Blocker über strukturierte Logs an das zentrale LogCenter (GitLab) und das Issue-System (`A2A-SIN-GitHub-Issues`) gemeldet, sodass das _gesamte_ Agenten-Team (die Workforce) das Problem lösen kann.
...
Hängt mit dem `gitlab_logcenter.py` und der `skylight-cli_screenshot_to_file` Funktion zusammen. Jeder Fehler-Status _muss_ bebildert werden.
