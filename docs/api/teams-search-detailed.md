# Teams Search API Endpoint — Detailed Reference

> **Method:** POST | **Resource:** Teams | **Action:** Search | **Version:** v1
...
Detailed reference for the teams search endpoint.
...
```bash
curl -X POST https://api.opensin.ai/api/v1/teams/search \
...
response = requests.post(
    "https://api.opensin.ai/api/v1/teams/search",
    headers={
...
```go
req, _ := http.NewRequest("POST", "https://api.opensin.ai/api/v1/teams/search", nil)
...
curl -X POST http://localhost:3000/api/v1/teams/search

# Test with staging
curl -X POST https://staging-api.opensin.ai/api/v1/teams/search
```
