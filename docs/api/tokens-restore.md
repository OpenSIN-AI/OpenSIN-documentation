# Tokens Restore — API Reference

> **Endpoint:** `POST /api/v1/tokens`/{id} | **Method:** POST | **Version:** 1.0

## Overview

Detailed API reference for the tokens restore endpoint.
...
```bash
curl -X POST https://api.opensin.ai/api/v1/tokens/{id} \
...
response = requests.post(
    "https://api.opensin.ai/api/v1/tokens/{id}",
    headers={
...
curl -X POST http://localhost:3000/api/v1/tokens/{id}

# Test with staging
curl -X POST https://staging-api.opensin.ai/api/v1/tokens/{id}
```

## Related Endpoints

- [Tokens List](./tokens-list.md)
- [Tokens Health](./tokens-health.md)
- [Tokens Metrics](./tokens-metrics.md)
