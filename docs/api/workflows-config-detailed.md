# Workflows Config API Endpoint — Detailed Reference

> **Method:** GET | **Resource:** Workflows | **Action:** Config | **Version:** v1
...
Detailed reference for the workflows config endpoint.
...
```bash
curl -X GET https://api.opensin.ai/api/v1/workflows/config \
...
response = requests.get(
    "https://api.opensin.ai/api/v1/workflows/config",
    headers={
...
```go
req, _ := http.NewRequest("GET", "https://api.opensin.ai/api/v1/workflows/config", nil)
...
curl -X GET http://localhost:3000/api/v1/workflows/config

# Test with staging
curl -X GET https://staging-api.opensin.ai/api/v1/workflows/config
```
