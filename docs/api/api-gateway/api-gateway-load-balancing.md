...
This document covers api gateway load balancing for the OpenSIN-AI API Gateway.

## Configuration

```yaml
api_gateway:
  api_gateway_load_balancing:
    enabled: true
    settings:
      timeout: 30s
      retries: 3
...
### Step 1: Setup
Configure the api gateway load balancing in the API Gateway.
