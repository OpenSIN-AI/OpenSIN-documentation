...
This document covers api gateway edge computing for the OpenSIN-AI API Gateway.

## Configuration

```yaml
api_gateway:
  api_gateway_edge_computing:
    enabled: true
    settings:
      timeout: 30s
      retries: 3
...
### Step 1: Setup
Configure the api gateway edge computing in the API Gateway.
