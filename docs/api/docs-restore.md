# Docs Restore — API Reference

> **Endpoint:** `POST /api/v1/docs`/{id} | **Method:** POST | **Version:** 1.0

## Overview

Detailed API reference for the docs restore endpoint.
...
```bash
curl -X POST https://api.opensin.ai/api/v1/docs/{id} \
...
curl -X POST http://localhost:3000/api/v1/docs/{id}

# Test with staging
curl -X POST https://staging-api.opensin.ai/api/v1/docs/{id}
```
