# Auth Delete — API Reference

> **Endpoint:** `DELETE /api/v1/auth`/{id} | **Method:** DELETE | **Version:** 1.0

## Overview

Detailed API reference for the auth delete endpoint.
...
```bash
curl -X DELETE https://api.opensin.ai/api/v1/auth/{id} \
...
response = requests.delete(
    "https://api.opensin.ai/api/v1/auth/{id}",
    headers={
...
curl -X DELETE http://localhost:3000/api/v1/auth/{id}

# Test with staging
curl -X DELETE https://staging-api.opensin.ai/api/v1/auth/{id}
```

## Related Endpoints

- [Auth List](./auth-list.md)
- [Auth Health](./auth-health.md)
- [Auth Metrics](./auth-metrics.md)
