# Auth Restore — API Reference

> **Endpoint:** `POST /api/v1/auth`/{id} | **Method:** POST | **Version:** 1.0

## Overview

Detailed API reference for the auth restore endpoint.
...
```bash
curl -X POST https://api.opensin.ai/api/v1/auth/{id} \
...
curl -X POST http://localhost:3000/api/v1/auth/{id}

# Test with staging
curl -X POST https://staging-api.opensin.ai/api/v1/auth/{id}
```
