# Tokens Patch — API Reference

> **Endpoint:** `PATCH /api/v1/tokens`/{id} | **Method:** PATCH | **Version:** 1.0

## Overview

Detailed API reference for the tokens patch endpoint.
...
```bash
curl -X PATCH https://api.opensin.ai/api/v1/tokens/{id} \
...
curl -X PATCH http://localhost:3000/api/v1/tokens/{id}

# Test with staging
curl -X PATCH https://staging-api.opensin.ai/api/v1/tokens/{id}
```
