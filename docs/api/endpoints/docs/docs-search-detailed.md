# Docs Search API Endpoint — Detailed Reference

> **Method:** POST | **Resource:** Docs | **Action:** Search | **Version:** v1
...
Detailed reference for the docs search endpoint.
...
```bash
curl -X POST https://api.opensin.ai/api/v1/docs/search \
...
response = requests.post(
    "https://api.opensin.ai/api/v1/docs/search",
    headers={
...
```go
req, _ := http.NewRequest("POST", "https://api.opensin.ai/api/v1/docs/search", nil)
...
curl -X POST http://localhost:3000/api/v1/docs/search

# Test with staging
curl -X POST https://staging-api.opensin.ai/api/v1/docs/search
```
