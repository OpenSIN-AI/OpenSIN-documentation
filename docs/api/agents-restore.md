# Agents Restore — API Reference

> **Endpoint:** `POST /api/v1/agents`/{id} | **Method:** POST | **Version:** 1.0

## Overview

Detailed API reference for the agents restore endpoint.
...
```bash
curl -X POST https://api.opensin.ai/api/v1/agents/{id} \
...
curl -X POST http://localhost:3000/api/v1/agents/{id}

# Test with staging
curl -X POST https://staging-api.opensin.ai/api/v1/agents/{id}
```
