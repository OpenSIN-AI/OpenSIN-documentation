# Repos Search API Endpoint — Detailed Reference

> **Method:** POST | **Resource:** Repos | **Action:** Search | **Version:** v1

## Endpoint

```
POST /api/v1/repos/search
```

## Description

Detailed reference for the repos search endpoint.
...
```bash
curl -X POST https://api.opensin.ai/api/v1/repos/search \
...
response = requests.post(
    "https://api.opensin.ai/api/v1/repos/search",
    headers={
...
```go
req, _ := http.NewRequest("POST", "https://api.opensin.ai/api/v1/repos/search", nil)
...
curl -X POST http://localhost:3000/api/v1/repos/search

# Test with staging
curl -X POST https://staging-api.opensin.ai/api/v1/repos/search
```
