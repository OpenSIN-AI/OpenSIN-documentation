# Auth Search API Endpoint — Detailed Reference

> **Method:** POST | **Resource:** Auth | **Action:** Search | **Version:** v1

## Endpoint

```
POST /api/v1/auth/search
```

## Description

Detailed reference for the auth search endpoint.
...
```bash
curl -X POST https://api.opensin.ai/api/v1/auth/search \
...
response = requests.post(
    "https://api.opensin.ai/api/v1/auth/search",
    headers={
...
```go
req, _ := http.NewRequest("POST", "https://api.opensin.ai/api/v1/auth/search", nil)
...
curl -X POST http://localhost:3000/api/v1/auth/search

# Test with staging
curl -X POST https://staging-api.opensin.ai/api/v1/auth/search
```

## Related Endpoints

- [Auth List](./auth-list-detailed.md)
- [Auth Health](./auth-health-detailed.md)
