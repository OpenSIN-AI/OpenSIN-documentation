# Mcps Config API Endpoint — Detailed Reference

> **Method:** GET | **Resource:** Mcps | **Action:** Config | **Version:** v1

## Endpoint

```
GET /api/v1/mcps/config
```

## Description

Detailed reference for the mcps config endpoint.
...
```bash
curl -X GET https://api.opensin.ai/api/v1/mcps/config \
...
response = requests.get(
    "https://api.opensin.ai/api/v1/mcps/config",
    headers={
...
```go
req, _ := http.NewRequest("GET", "https://api.opensin.ai/api/v1/mcps/config", nil)
...
curl -X GET http://localhost:3000/api/v1/mcps/config

# Test with staging
curl -X GET https://staging-api.opensin.ai/api/v1/mcps/config
```
