# Tokens Search API Endpoint — Detailed Reference

> **Method:** POST | **Resource:** Tokens | **Action:** Search | **Version:** v1
...
Detailed reference for the tokens search endpoint.
...
```bash
curl -X POST https://api.opensin.ai/api/v1/tokens/search \
...
response = requests.post(
    "https://api.opensin.ai/api/v1/tokens/search",
    headers={
...
```go
req, _ := http.NewRequest("POST", "https://api.opensin.ai/api/v1/tokens/search", nil)
...
curl -X POST http://localhost:3000/api/v1/tokens/search

# Test with staging
curl -X POST https://staging-api.opensin.ai/api/v1/tokens/search
```
