# Sin Audit Agent — Troubleshooting Guide

> **Agent:** sin-audit-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-audit-agent`
2. Restart agent: `opencode run --agent "sin-audit-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-audit-agent.log`
...
1. Collect logs: `journalctl -u sin-audit-agent --no-pager > /tmp/sin-audit-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-audit-agent/config.yaml > /tmp/sin-audit-agent-config.txt`
