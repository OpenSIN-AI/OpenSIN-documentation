# Sin Patent Agent — Troubleshooting Guide

> **Agent:** sin-patent-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-patent-agent`
2. Restart agent: `opencode run --agent "sin-patent-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-patent-agent.log`
...
1. Collect logs: `journalctl -u sin-patent-agent --no-pager > /tmp/sin-patent-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-patent-agent/config.yaml > /tmp/sin-patent-agent-config.txt`
