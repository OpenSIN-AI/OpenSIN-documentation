# Sin Facebook Agent — Troubleshooting Guide

> **Agent:** sin-facebook-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-facebook-agent`
2. Restart agent: `opencode run --agent "sin-facebook-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-facebook-agent.log`
...
1. Collect logs: `journalctl -u sin-facebook-agent --no-pager > /tmp/sin-facebook-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-facebook-agent/config.yaml > /tmp/sin-facebook-agent-config.txt`
