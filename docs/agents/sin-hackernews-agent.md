# Sin Hackernews Agent — Troubleshooting Guide

> **Agent:** sin-hackernews-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-hackernews-agent`
2. Restart agent: `opencode run --agent "sin-hackernews-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-hackernews-agent.log`
...
1. Collect logs: `journalctl -u sin-hackernews-agent --no-pager > /tmp/sin-hackernews-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-hackernews-agent/config.yaml > /tmp/sin-hackernews-agent-config.txt`
