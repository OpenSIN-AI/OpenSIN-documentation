# Sin Freelancer Agent — Troubleshooting Guide

> **Agent:** sin-freelancer-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-freelancer-agent`
2. Restart agent: `opencode run --agent "sin-freelancer-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-freelancer-agent.log`
...
1. Collect logs: `journalctl -u sin-freelancer-agent --no-pager > /tmp/sin-freelancer-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-freelancer-agent/config.yaml > /tmp/sin-freelancer-agent-config.txt`
