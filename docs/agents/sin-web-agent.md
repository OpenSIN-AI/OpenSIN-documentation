# Sin Web Agent — Troubleshooting Guide

> **Agent:** sin-web-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-web-agent`
2. Restart agent: `opencode run --agent "sin-web-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-web-agent.log`
...
1. Collect logs: `journalctl -u sin-web-agent --no-pager > /tmp/sin-web-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-web-agent/config.yaml > /tmp/sin-web-agent-config.txt`
