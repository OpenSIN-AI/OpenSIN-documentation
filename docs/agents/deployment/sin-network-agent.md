# Sin Network Agent — Deployment Guide

> **Agent:** sin-network-agent | **Category:** A2A Agent Deployment | **Version:** 1.0 | **Status:** Active

## Overview

This guide provides step-by-step deployment instructions for the Sin Network Agent.
...
```bash
git clone git@github.com:OpenSIN-AI/sin-network-agent.git
cd sin-network-agent
```
...
curl -X POST http://92.5.60.87:8006/rest/v1/agent_registry \
  ...
  -d '{"name": "sin-network-agent", "status": "active", "model": "openrouter/qwen/qwen3.6-plus:free"}'
```
...
huggingface-cli repo create sin-network-agent --type space --space_sdk docker
git push https://huggingface.co/spaces/delqhi/sin-network-agent
```
...
curl http://92.5.60.87:8006/rest/v1/agent_registry?name=eq.sin-network-agent \
  -H "apikey: $SUPABASE_KEY"
```
...
- [Agent Reference Card](../reference-cards/sin-network-agent.md)
- [Integration Guide](../../integrations/agents/sin-network-agent.md)
- [Team Playbook](../team-playbooks/)
- [Agent Configuration](../fleet/configs/sin-network-agent.md)
