# Sin Instagram Agent — Deployment Guide

> **Agent:** sin-instagram-agent | **Category:** A2A Agent Deployment | **Version:** 1.0 | **Status:** Active

## Overview

This guide provides step-by-step deployment instructions for the Sin Instagram Agent.
...
```bash
git clone git@github.com:OpenSIN-AI/sin-instagram-agent.git
cd sin-instagram-agent
```
...
curl -X POST http://92.5.60.87:8006/rest/v1/agent_registry \
  ...
  -d '{"name": "sin-instagram-agent", "status": "active", "model": "openrouter/qwen/qwen3.6-plus:free"}'
```
...
huggingface-cli repo create sin-instagram-agent --type space --space_sdk docker
git push https://huggingface.co/spaces/delqhi/sin-instagram-agent
```
...
curl http://92.5.60.87:8006/rest/v1/agent_registry?name=eq.sin-instagram-agent \
  -H "apikey: $SUPABASE_KEY"
```
...
- [Agent Reference Card](../reference-cards/sin-instagram-agent.md)
- [Integration Guide](../../integrations/agents/sin-instagram-agent.md)
- [Team Playbook](../team-playbooks/)
- [Agent Configuration](../fleet/configs/sin-instagram-agent.md)
