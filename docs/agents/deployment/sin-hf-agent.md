# Sin Hf Agent — Deployment Guide

> **Agent:** sin-hf-agent | **Category:** A2A Agent Deployment | **Version:** 1.0 | **Status:** Active

## Overview

This guide provides step-by-step deployment instructions for the Sin Hf Agent.
...
```bash
git clone git@github.com:OpenSIN-AI/sin-hf-agent.git
cd sin-hf-agent
```
...
curl -X POST http://92.5.60.87:8006/rest/v1/agent_registry \
  ...
  -d '{"name": "sin-hf-agent", "status": "active", "model": "openrouter/qwen/qwen3.6-plus:free"}'
```
...
huggingface-cli repo create sin-hf-agent --type space --space_sdk docker
git push https://huggingface.co/spaces/delqhi/sin-hf-agent
```
...
curl http://92.5.60.87:8006/rest/v1/agent_registry?name=eq.sin-hf-agent \
  -H "apikey: $SUPABASE_KEY"
```
...
- [Agent Reference Card](../reference-cards/sin-hf-agent.md)
- [Integration Guide](../../integrations/agents/sin-hf-agent.md)
- [Team Playbook](../team-playbooks/)
- [Agent Configuration](../fleet/configs/sin-hf-agent.md)
