# Sin Notifications Agent — Deployment Guide

> **Agent:** sin-notifications-agent | **Category:** A2A Agent Deployment | **Version:** 1.0 | **Status:** Active

## Overview

This guide provides step-by-step deployment instructions for the Sin Notifications Agent.
...
```bash
git clone git@github.com:OpenSIN-AI/sin-notifications-agent.git
cd sin-notifications-agent
```
...
curl -X POST http://92.5.60.87:8006/rest/v1/agent_registry \
  ...
  -d '{"name": "sin-notifications-agent", "status": "active", "model": "openrouter/qwen/qwen3.6-plus:free"}'
```
...
huggingface-cli repo create sin-notifications-agent --type space --space_sdk docker
git push https://huggingface.co/spaces/delqhi/sin-notifications-agent
```
...
curl http://92.5.60.87:8006/rest/v1/agent_registry?name=eq.sin-notifications-agent \
  -H "apikey: $SUPABASE_KEY"
```
...
- [Agent Reference Card](../reference-cards/sin-notifications-agent.md)
- [Integration Guide](../../integrations/agents/sin-notifications-agent.md)
- [Team Playbook](../team-playbooks/)
- [Agent Configuration](../fleet/configs/sin-notifications-agent.md)
