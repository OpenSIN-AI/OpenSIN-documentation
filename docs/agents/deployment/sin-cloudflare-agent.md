# Sin Cloudflare Agent — Deployment Guide

> **Agent:** sin-cloudflare-agent | **Category:** A2A Agent Deployment | **Version:** 1.0 | **Status:** Active

## Overview

This guide provides step-by-step deployment instructions for the Sin Cloudflare Agent.
...
```bash
git clone git@github.com:OpenSIN-AI/sin-cloudflare-agent.git
cd sin-cloudflare-agent
```
...
curl -X POST http://92.5.60.87:8006/rest/v1/agent_registry \
  ...
  -d '{"name": "sin-cloudflare-agent", "status": "active", "model": "openrouter/qwen/qwen3.6-plus:free"}'
```
...
huggingface-cli repo create sin-cloudflare-agent --type space --space_sdk docker
git push https://huggingface.co/spaces/delqhi/sin-cloudflare-agent
```
...
curl http://92.5.60.87:8006/rest/v1/agent_registry?name=eq.sin-cloudflare-agent \
  -H "apikey: $SUPABASE_KEY"
```
...
- [Agent Reference Card](../reference-cards/sin-cloudflare-agent.md)
- [Integration Guide](../../integrations/agents/sin-cloudflare-agent.md)
- [Team Playbook](../team-playbooks/)
- [Agent Configuration](../fleet/configs/sin-cloudflare-agent.md)
