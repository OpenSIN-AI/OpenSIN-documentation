# Sin Email Agent — Deployment Guide

> **Agent:** sin-email-agent | **Category:** A2A Agent Deployment | **Version:** 1.0 | **Status:** Active

## Overview

This guide provides step-by-step deployment instructions for the Sin Email Agent.
...
```bash
git clone git@github.com:OpenSIN-AI/sin-email-agent.git
cd sin-email-agent
```
...
curl -X POST http://92.5.60.87:8006/rest/v1/agent_registry \
  ...
  -d '{"name": "sin-email-agent", "status": "active", "model": "openrouter/qwen/qwen3.6-plus:free"}'
```
...
huggingface-cli repo create sin-email-agent --type space --space_sdk docker
git push https://huggingface.co/spaces/delqhi/sin-email-agent
```
...
curl http://92.5.60.87:8006/rest/v1/agent_registry?name=eq.sin-email-agent \
  -H "apikey: $SUPABASE_KEY"
```
...
- [Agent Reference Card](../reference-cards/sin-email-agent.md)
- [Integration Guide](../../integrations/agents/sin-email-agent.md)
- [Team Playbook](../team-playbooks/)
- [Agent Configuration](../fleet/configs/sin-email-agent.md)
