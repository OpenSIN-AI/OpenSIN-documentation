# Sin Patent Agent — Deployment Guide

> **Agent:** sin-patent-agent | **Category:** A2A Agent Deployment | **Version:** 1.0 | **Status:** Active

## Overview

This guide provides step-by-step deployment instructions for the Sin Patent Agent.
...
```bash
git clone git@github.com:OpenSIN-AI/sin-patent-agent.git
cd sin-patent-agent
```
...
curl -X POST http://92.5.60.87:8006/rest/v1/agent_registry \
  ...
  -d '{"name": "sin-patent-agent", "status": "active", "model": "openrouter/qwen/qwen3.6-plus:free"}'
```
...
huggingface-cli repo create sin-patent-agent --type space --space_sdk docker
git push https://huggingface.co/spaces/delqhi/sin-patent-agent
```
...
curl http://92.5.60.87:8006/rest/v1/agent_registry?name=eq.sin-patent-agent \
  -H "apikey: $SUPABASE_KEY"
```
...
- [Agent Reference Card](../reference-cards/sin-patent-agent.md)
- [Integration Guide](../../integrations/agents/sin-patent-agent.md)
- [Team Playbook](../team-playbooks/)
- [Agent Configuration](../fleet/configs/sin-patent-agent.md)
