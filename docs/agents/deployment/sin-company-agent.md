# Sin Company Agent — Deployment Guide

> **Agent:** sin-company-agent | **Category:** A2A Agent Deployment | **Version:** 1.0 | **Status:** Active

## Overview

This guide provides step-by-step deployment instructions for the Sin Company Agent.
...
```bash
git clone git@github.com:OpenSIN-AI/sin-company-agent.git
cd sin-company-agent
```
...
curl -X POST http://92.5.60.87:8006/rest/v1/agent_registry \
  ...
  -d '{"name": "sin-company-agent", "status": "active", "model": "openrouter/qwen/qwen3.6-plus:free"}'
```
...
huggingface-cli repo create sin-company-agent --type space --space_sdk docker
git push https://huggingface.co/spaces/delqhi/sin-company-agent
```
...
curl http://92.5.60.87:8006/rest/v1/agent_registry?name=eq.sin-company-agent \
  -H "apikey: $SUPABASE_KEY"
```
...
- [Agent Reference Card](../reference-cards/sin-company-agent.md)
- [Integration Guide](../../integrations/agents/sin-company-agent.md)
- [Team Playbook](../team-playbooks/)
- [Agent Configuration](../fleet/configs/sin-company-agent.md)
