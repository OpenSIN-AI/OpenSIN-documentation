# Sin Auth Agent — Deployment Guide

> **Agent:** sin-auth-agent | **Category:** A2A Agent Deployment | **Version:** 1.0 | **Status:** Active

## Overview

This guide provides step-by-step deployment instructions for the Sin Auth Agent.
...
```bash
git clone git@github.com:OpenSIN-AI/sin-auth-agent.git
cd sin-auth-agent
```
...
curl -X POST http://92.5.60.87:8006/rest/v1/agent_registry \
  ...
  -d '{"name": "sin-auth-agent", "status": "active", "model": "openrouter/qwen/qwen3.6-plus:free"}'
```
...
huggingface-cli repo create sin-auth-agent --type space --space_sdk docker
git push https://huggingface.co/spaces/delqhi/sin-auth-agent
```
...
curl http://92.5.60.87:8006/rest/v1/agent_registry?name=eq.sin-auth-agent \
  -H "apikey: $SUPABASE_KEY"
```
...
- [Agent Reference Card](../reference-cards/sin-auth-agent.md)
- [Integration Guide](../../integrations/agents/sin-auth-agent.md)
- [Team Playbook](../team-playbooks/)
- [Agent Configuration](../fleet/configs/sin-auth-agent.md)
