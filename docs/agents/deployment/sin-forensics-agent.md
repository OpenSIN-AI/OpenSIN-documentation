# Sin Forensics Agent — Deployment Guide

> **Agent:** sin-forensics-agent | **Category:** A2A Agent Deployment | **Version:** 1.0 | **Status:** Active

## Overview

This guide provides step-by-step deployment instructions for the Sin Forensics Agent.
...
```bash
git clone git@github.com:OpenSIN-AI/sin-forensics-agent.git
cd sin-forensics-agent
```
...
curl -X POST http://92.5.60.87:8006/rest/v1/agent_registry \
  ...
  -d '{"name": "sin-forensics-agent", "status": "active", "model": "openrouter/qwen/qwen3.6-plus:free"}'
```
...
huggingface-cli repo create sin-forensics-agent --type space --space_sdk docker
git push https://huggingface.co/spaces/delqhi/sin-forensics-agent
```
...
curl http://92.5.60.87:8006/rest/v1/agent_registry?name=eq.sin-forensics-agent \
  -H "apikey: $SUPABASE_KEY"
```
...
- [Agent Reference Card](../reference-cards/sin-forensics-agent.md)
- [Integration Guide](../../integrations/agents/sin-forensics-agent.md)
- [Team Playbook](../team-playbooks/)
- [Agent Configuration](../fleet/configs/sin-forensics-agent.md)
