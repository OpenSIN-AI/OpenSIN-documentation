# Sin Creator Agent — Deployment Guide

> **Agent:** sin-creator-agent | **Category:** A2A Agent Deployment | **Version:** 1.0 | **Status:** Active

## Overview

This guide provides step-by-step deployment instructions for the Sin Creator Agent.
...
```bash
git clone git@github.com:OpenSIN-AI/sin-creator-agent.git
cd sin-creator-agent
```
...
curl -X POST http://92.5.60.87:8006/rest/v1/agent_registry \
  ...
  -d '{"name": "sin-creator-agent", "status": "active", "model": "openrouter/qwen/qwen3.6-plus:free"}'
```
...
huggingface-cli repo create sin-creator-agent --type space --space_sdk docker
git push https://huggingface.co/spaces/delqhi/sin-creator-agent
```
...
curl http://92.5.60.87:8006/rest/v1/agent_registry?name=eq.sin-creator-agent \
  -H "apikey: $SUPABASE_KEY"
```
...
- [Agent Reference Card](../reference-cards/sin-creator-agent.md)
- [Integration Guide](../../integrations/agents/sin-creator-agent.md)
- [Team Playbook](../team-playbooks/)
- [Agent Configuration](../fleet/configs/sin-creator-agent.md)
