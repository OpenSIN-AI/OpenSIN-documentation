# Sin Tiktokshop Agent — Deployment Guide

> **Agent:** sin-tiktokshop-agent | **Category:** A2A Agent Deployment | **Version:** 1.0 | **Status:** Active

## Overview

This guide provides step-by-step deployment instructions for the Sin Tiktokshop Agent.
...
```bash
git clone git@github.com:OpenSIN-AI/sin-tiktokshop-agent.git
cd sin-tiktokshop-agent
```
...
curl -X POST http://92.5.60.87:8006/rest/v1/agent_registry \
  ...
  -d '{"name": "sin-tiktokshop-agent", "status": "active", "model": "openrouter/qwen/qwen3.6-plus:free"}'
```
...
huggingface-cli repo create sin-tiktokshop-agent --type space --space_sdk docker
git push https://huggingface.co/spaces/delqhi/sin-tiktokshop-agent
```
...
curl http://92.5.60.87:8006/rest/v1/agent_registry?name=eq.sin-tiktokshop-agent \
  -H "apikey: $SUPABASE_KEY"
```
...
- [Agent Reference Card](../reference-cards/sin-tiktokshop-agent.md)
- [Integration Guide](../../integrations/agents/sin-tiktokshop-agent.md)
- [Team Playbook](../team-playbooks/)
- [Agent Configuration](../fleet/configs/sin-tiktokshop-agent.md)
