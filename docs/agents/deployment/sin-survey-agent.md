# Sin Survey Agent — Deployment Guide

> **Agent:** sin-survey-agent | **Category:** A2A Agent Deployment | **Version:** 1.0 | **Status:** Active

## Overview

This guide provides step-by-step deployment instructions for the Sin Survey Agent.
...
```bash
git clone git@github.com:OpenSIN-AI/sin-survey-agent.git
cd sin-survey-agent
```
...
curl -X POST http://92.5.60.87:8006/rest/v1/agent_registry \
  ...
  -d '{"name": "sin-survey-agent", "status": "active", "model": "openrouter/qwen/qwen3.6-plus:free"}'
```
...
huggingface-cli repo create sin-survey-agent --type space --space_sdk docker
git push https://huggingface.co/spaces/delqhi/sin-survey-agent
```
...
curl http://92.5.60.87:8006/rest/v1/agent_registry?name=eq.sin-survey-agent \
  -H "apikey: $SUPABASE_KEY"
```
...
- [Agent Reference Card](../reference-cards/sin-survey-agent.md)
- [Integration Guide](../../integrations/agents/sin-survey-agent.md)
- [Team Playbook](../team-playbooks/)
- [Agent Configuration](../fleet/configs/sin-survey-agent.md)
