# Sin Finance Agent — Deployment Guide

> **Agent:** sin-finance-agent | **Category:** A2A Agent Deployment | **Version:** 1.0 | **Status:** Active

## Overview

This guide provides step-by-step deployment instructions for the Sin Finance Agent.
...
```bash
git clone git@github.com:OpenSIN-AI/sin-finance-agent.git
cd sin-finance-agent
```
...
curl -X POST http://92.5.60.87:8006/rest/v1/agent_registry \
  ...
  -d '{"name": "sin-finance-agent", "status": "active", "model": "openrouter/qwen/qwen3.6-plus:free"}'
```
...
huggingface-cli repo create sin-finance-agent --type space --space_sdk docker
git push https://huggingface.co/spaces/delqhi/sin-finance-agent
```
...
curl http://92.5.60.87:8006/rest/v1/agent_registry?name=eq.sin-finance-agent \
  -H "apikey: $SUPABASE_KEY"
```
...
- [Agent Reference Card](../reference-cards/sin-finance-agent.md)
- [Integration Guide](../../integrations/agents/sin-finance-agent.md)
- [Team Playbook](../team-playbooks/)
- [Agent Configuration](../fleet/configs/sin-finance-agent.md)
