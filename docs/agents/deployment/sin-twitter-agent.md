# Sin Twitter Agent — Deployment Guide

> **Agent:** sin-twitter-agent | **Category:** A2A Agent Deployment | **Version:** 1.0 | **Status:** Active

## Overview

This guide provides step-by-step deployment instructions for the Sin Twitter Agent.
...
```bash
git clone git@github.com:OpenSIN-AI/sin-twitter-agent.git
cd sin-twitter-agent
```
...
curl -X POST http://92.5.60.87:8006/rest/v1/agent_registry \
  ...
  -d '{"name": "sin-twitter-agent", "status": "active", "model": "openrouter/qwen/qwen3.6-plus:free"}'
```
...
huggingface-cli repo create sin-twitter-agent --type space --space_sdk docker
git push https://huggingface.co/spaces/delqhi/sin-twitter-agent
```
...
curl http://92.5.60.87:8006/rest/v1/agent_registry?name=eq.sin-twitter-agent \
  -H "apikey: $SUPABASE_KEY"
```
...
- [Agent Reference Card](../reference-cards/sin-twitter-agent.md)
- [Integration Guide](../../integrations/agents/sin-twitter-agent.md)
- [Team Playbook](../team-playbooks/)
- [Agent Configuration](../fleet/configs/sin-twitter-agent.md)
