# Sin Backup Agent — Deployment Guide

> **Agent:** sin-backup-agent | **Category:** A2A Agent Deployment | **Version:** 1.0 | **Status:** Active

## Overview

This guide provides step-by-step deployment instructions for the Sin Backup Agent.
...
```bash
git clone git@github.com:OpenSIN-AI/sin-backup-agent.git
cd sin-backup-agent
```
...
curl -X POST http://92.5.60.87:8006/rest/v1/agent_registry \
  ...
  -d '{"name": "sin-backup-agent", "status": "active", "model": "openrouter/qwen/qwen3.6-plus:free"}'
```
...
huggingface-cli repo create sin-backup-agent --type space --space_sdk docker
git push https://huggingface.co/spaces/delqhi/sin-backup-agent
```
...
curl http://92.5.60.87:8006/rest/v1/agent_registry?name=eq.sin-backup-agent \
  -H "apikey: $SUPABASE_KEY"
```
...
- [Agent Reference Card](../reference-cards/sin-backup-agent.md)
- [Integration Guide](../../integrations/agents/sin-backup-agent.md)
- [Team Playbook](../team-playbooks/)
- [Agent Configuration](../fleet/configs/sin-backup-agent.md)
