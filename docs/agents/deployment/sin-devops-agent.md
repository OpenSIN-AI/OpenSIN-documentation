# Sin Devops Agent — Deployment Guide

> **Agent:** sin-devops-agent | **Category:** A2A Agent Deployment | **Version:** 1.0 | **Status:** Active

## Overview

This guide provides step-by-step deployment instructions for the Sin Devops Agent.
...
```bash
git clone git@github.com:OpenSIN-AI/sin-devops-agent.git
cd sin-devops-agent
```
...
curl -X POST http://92.5.60.87:8006/rest/v1/agent_registry \
  ...
  -d '{"name": "sin-devops-agent", "status": "active", "model": "openrouter/qwen/qwen3.6-plus:free"}'
```
...
huggingface-cli repo create sin-devops-agent --type space --space_sdk docker
git push https://huggingface.co/spaces/delqhi/sin-devops-agent
```
...
curl http://92.5.60.87:8006/rest/v1/agent_registry?name=eq.sin-devops-agent \
  -H "apikey: $SUPABASE_KEY"
```
...
- [Agent Reference Card](../reference-cards/sin-devops-agent.md)
- [Integration Guide](../../integrations/agents/sin-devops-agent.md)
- [Team Playbook](../team-playbooks/)
- [Agent Configuration](../fleet/configs/sin-devops-agent.md)
