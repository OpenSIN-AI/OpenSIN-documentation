# Sin Worker Agent — Deployment Guide

> **Agent:** sin-worker-agent | **Category:** A2A Agent Deployment | **Version:** 1.0 | **Status:** Active

## Overview

This guide provides step-by-step deployment instructions for the Sin Worker Agent.
...
```bash
git clone git@github.com:OpenSIN-AI/sin-worker-agent.git
cd sin-worker-agent
```
...
curl -X POST http://92.5.60.87:8006/rest/v1/agent_registry \
  ...
  -d '{"name": "sin-worker-agent", "status": "active", "model": "openrouter/qwen/qwen3.6-plus:free"}'
```
...
huggingface-cli repo create sin-worker-agent --type space --space_sdk docker
git push https://huggingface.co/spaces/delqhi/sin-worker-agent
```
...
curl http://92.5.60.87:8006/rest/v1/agent_registry?name=eq.sin-worker-agent \
  -H "apikey: $SUPABASE_KEY"
```
...
- [Agent Reference Card](../reference-cards/sin-worker-agent.md)
- [Integration Guide](../../integrations/agents/sin-worker-agent.md)
- [Team Playbook](../team-playbooks/)
- [Agent Configuration](../fleet/configs/sin-worker-agent.md)
