# Sin Crypto Agent — Deployment Guide

> **Agent:** sin-crypto-agent | **Category:** A2A Agent Deployment | **Version:** 1.0 | **Status:** Active

## Overview

This guide provides step-by-step deployment instructions for the Sin Crypto Agent.
...
```bash
git clone git@github.com:OpenSIN-AI/sin-crypto-agent.git
cd sin-crypto-agent
```
...
curl -X POST http://92.5.60.87:8006/rest/v1/agent_registry \
  ...
  -d '{"name": "sin-crypto-agent", "status": "active", "model": "openrouter/qwen/qwen3.6-plus:free"}'
```
...
huggingface-cli repo create sin-crypto-agent --type space --space_sdk docker
git push https://huggingface.co/spaces/delqhi/sin-crypto-agent
```
...
curl http://92.5.60.87:8006/rest/v1/agent_registry?name=eq.sin-crypto-agent \
  -H "apikey: $SUPABASE_KEY"
```
...
- [Agent Reference Card](../reference-cards/sin-crypto-agent.md)
- [Integration Guide](../../integrations/agents/sin-crypto-agent.md)
- [Team Playbook](../team-playbooks/)
- [Agent Configuration](../fleet/configs/sin-crypto-agent.md)
