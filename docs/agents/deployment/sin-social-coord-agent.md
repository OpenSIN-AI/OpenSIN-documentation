# Sin Social Coord Agent — Deployment Guide

> **Agent:** sin-social-coord-agent | **Category:** A2A Agent Deployment | **Version:** 1.0 | **Status:** Active

## Overview

This guide provides step-by-step deployment instructions for the Sin Social Coord Agent.
...
```bash
git clone git@github.com:OpenSIN-AI/sin-social-coord-agent.git
cd sin-social-coord-agent
```
...
curl -X POST http://92.5.60.87:8006/rest/v1/agent_registry \
  ...
  -d '{"name": "sin-social-coord-agent", "status": "active", "model": "openrouter/qwen/qwen3.6-plus:free"}'
```
...
huggingface-cli repo create sin-social-coord-agent --type space --space_sdk docker
git push https://huggingface.co/spaces/delqhi/sin-social-coord-agent
```
...
curl http://92.5.60.87:8006/rest/v1/agent_registry?name=eq.sin-social-coord-agent \
  -H "apikey: $SUPABASE_KEY"
```
...
- [Agent Reference Card](../reference-cards/sin-social-coord-agent.md)
- [Integration Guide](../../integrations/agents/sin-social-coord-agent.md)
- [Team Playbook](../team-playbooks/)
- [Agent Configuration](../fleet/configs/sin-social-coord-agent.md)
