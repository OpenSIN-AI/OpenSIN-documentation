# Sin Web Agent — Deployment Guide

> **Agent:** sin-web-agent | **Category:** A2A Agent Deployment | **Version:** 1.0 | **Status:** Active

## Overview

This guide provides step-by-step deployment instructions for the Sin Web Agent.
...
```bash
git clone git@github.com:OpenSIN-AI/sin-web-agent.git
cd sin-web-agent
```
...
curl -X POST http://92.5.60.87:8006/rest/v1/agent_registry \
  ...
  -d '{"name": "sin-web-agent", "status": "active", "model": "openrouter/qwen/qwen3.6-plus:free"}'
```
...
huggingface-cli repo create sin-web-agent --type space --space_sdk docker
git push https://huggingface.co/spaces/delqhi/sin-web-agent
```
...
curl http://92.5.60.87:8006/rest/v1/agent_registry?name=eq.sin-web-agent \
  -H "apikey: $SUPABASE_KEY"
```
...
- [Agent Reference Card](../reference-cards/sin-web-agent.md)
- [Integration Guide](../../integrations/agents/sin-web-agent.md)
- [Team Playbook](../team-playbooks/)
- [Agent Configuration](../fleet/configs/sin-web-agent.md)
