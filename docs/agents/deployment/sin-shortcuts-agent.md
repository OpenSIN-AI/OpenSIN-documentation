# Sin Shortcuts Agent — Deployment Guide

> **Agent:** sin-shortcuts-agent | **Category:** A2A Agent Deployment | **Version:** 1.0 | **Status:** Active

## Overview

This guide provides step-by-step deployment instructions for the Sin Shortcuts Agent.
...
```bash
git clone git@github.com:OpenSIN-AI/sin-shortcuts-agent.git
cd sin-shortcuts-agent
```
...
curl -X POST http://92.5.60.87:8006/rest/v1/agent_registry \
  ...
  -d '{"name": "sin-shortcuts-agent", "status": "active", "model": "openrouter/qwen/qwen3.6-plus:free"}'
```
...
huggingface-cli repo create sin-shortcuts-agent --type space --space_sdk docker
git push https://huggingface.co/spaces/delqhi/sin-shortcuts-agent
```
...
curl http://92.5.60.87:8006/rest/v1/agent_registry?name=eq.sin-shortcuts-agent \
  -H "apikey: $SUPABASE_KEY"
```
...
- [Agent Reference Card](../reference-cards/sin-shortcuts-agent.md)
- [Integration Guide](../../integrations/agents/sin-shortcuts-agent.md)
- [Team Playbook](../team-playbooks/)
- [Agent Configuration](../fleet/configs/sin-shortcuts-agent.md)
