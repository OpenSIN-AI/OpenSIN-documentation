# Sin Database Agent — Deployment Guide

> **Agent:** sin-database-agent | **Category:** A2A Agent Deployment | **Version:** 1.0 | **Status:** Active

## Overview

This guide provides step-by-step deployment instructions for the Sin Database Agent.
...
```bash
git clone git@github.com:OpenSIN-AI/sin-database-agent.git
cd sin-database-agent
```
...
curl -X POST http://92.5.60.87:8006/rest/v1/agent_registry \
  ...
  -d '{"name": "sin-database-agent", "status": "active", "model": "openrouter/qwen/qwen3.6-plus:free"}'
```
...
huggingface-cli repo create sin-database-agent --type space --space_sdk docker
git push https://huggingface.co/spaces/delqhi/sin-database-agent
```
...
curl http://92.5.60.87:8006/rest/v1/agent_registry?name=eq.sin-database-agent \
  -H "apikey: $SUPABASE_KEY"
```
...
- [Agent Reference Card](../reference-cards/sin-database-agent.md)
- [Integration Guide](../../integrations/agents/sin-database-agent.md)
- [Team Playbook](../team-playbooks/)
- [Agent Configuration](../fleet/configs/sin-database-agent.md)
