# Sin Mobile Device Agent — Deployment Guide

> **Agent:** sin-mobile-device-agent | **Category:** A2A Agent Deployment | **Version:** 1.0 | **Status:** Active

## Overview

This guide provides step-by-step deployment instructions for the Sin Mobile Device Agent.
...
```bash
git clone git@github.com:OpenSIN-AI/sin-mobile-device-agent.git
cd sin-mobile-device-agent
```
...
curl -X POST http://92.5.60.87:8006/rest/v1/agent_registry \
  ...
  -d '{"name": "sin-mobile-device-agent", "status": "active", "model": "openrouter/qwen/qwen3.6-plus:free"}'
```
...
huggingface-cli repo create sin-mobile-device-agent --type space --space_sdk docker
git push https://huggingface.co/spaces/delqhi/sin-mobile-device-agent
```
...
curl http://92.5.60.87:8006/rest/v1/agent_registry?name=eq.sin-mobile-device-agent \
  -H "apikey: $SUPABASE_KEY"
```
...
- [Agent Reference Card](../reference-cards/sin-mobile-device-agent.md)
- [Integration Guide](../../integrations/agents/sin-mobile-device-agent.md)
- [Team Playbook](../team-playbooks/)
- [Agent Configuration](../fleet/configs/sin-mobile-device-agent.md)
