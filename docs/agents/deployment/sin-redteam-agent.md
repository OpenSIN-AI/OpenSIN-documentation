# Sin Redteam Agent — Deployment Guide

> **Agent:** sin-redteam-agent | **Category:** A2A Agent Deployment | **Version:** 1.0 | **Status:** Active

## Overview

This guide provides step-by-step deployment instructions for the Sin Redteam Agent.
...
```bash
git clone git@github.com:OpenSIN-AI/sin-redteam-agent.git
cd sin-redteam-agent
```
...
curl -X POST http://92.5.60.87:8006/rest/v1/agent_registry \
  ...
  -d '{"name": "sin-redteam-agent", "status": "active", "model": "openrouter/qwen/qwen3.6-plus:free"}'
```
...
huggingface-cli repo create sin-redteam-agent --type space --space_sdk docker
git push https://huggingface.co/spaces/delqhi/sin-redteam-agent
```
...
curl http://92.5.60.87:8006/rest/v1/agent_registry?name=eq.sin-redteam-agent \
  -H "apikey: $SUPABASE_KEY"
```
...
- [Agent Reference Card](../reference-cards/sin-redteam-agent.md)
- [Integration Guide](../../integrations/agents/sin-redteam-agent.md)
- [Team Playbook](../team-playbooks/)
- [Agent Configuration](../fleet/configs/sin-redteam-agent.md)
