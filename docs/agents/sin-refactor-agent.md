# Sin Refactor Agent — Troubleshooting Guide

> **Agent:** sin-refactor-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-refactor-agent`
2. Restart agent: `opencode run --agent "sin-refactor-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-refactor-agent.log`
...
1. Collect logs: `journalctl -u sin-refactor-agent --no-pager > /tmp/sin-refactor-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-refactor-agent/config.yaml > /tmp/sin-refactor-agent-config.txt`
