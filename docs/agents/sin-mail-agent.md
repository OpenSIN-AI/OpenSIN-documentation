# Sin Mail Agent — Troubleshooting Guide

> **Agent:** sin-mail-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-mail-agent`
2. Restart agent: `opencode run --agent "sin-mail-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-mail-agent.log`
...
1. Collect logs: `journalctl -u sin-mail-agent --no-pager > /tmp/sin-mail-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-mail-agent/config.yaml > /tmp/sin-mail-agent-config.txt`
