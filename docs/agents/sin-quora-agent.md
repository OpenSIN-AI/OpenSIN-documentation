# Sin Quora Agent — Troubleshooting Guide

> **Agent:** sin-quora-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-quora-agent`
2. Restart agent: `opencode run --agent "sin-quora-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-quora-agent.log`
...
1. Collect logs: `journalctl -u sin-quora-agent --no-pager > /tmp/sin-quora-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-quora-agent/config.yaml > /tmp/sin-quora-agent-config.txt`
