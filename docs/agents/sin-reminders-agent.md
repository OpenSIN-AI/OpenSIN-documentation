# Sin Reminders Agent — Troubleshooting Guide

> **Agent:** sin-reminders-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-reminders-agent`
2. Restart agent: `opencode run --agent "sin-reminders-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-reminders-agent.log`
...
1. Collect logs: `journalctl -u sin-reminders-agent --no-pager > /tmp/sin-reminders-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-reminders-agent/config.yaml > /tmp/sin-reminders-agent-config.txt`
