# Sin Network Agent — Troubleshooting Guide

> **Agent:** sin-network-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-network-agent`
2. Restart agent: `opencode run --agent "sin-network-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-network-agent.log`
...
1. Collect logs: `journalctl -u sin-network-agent --no-pager > /tmp/sin-network-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-network-agent/config.yaml > /tmp/sin-network-agent-config.txt`
