# Sin Devicecontrol Agent — Troubleshooting Guide

> **Agent:** sin-devicecontrol-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-devicecontrol-agent`
2. Restart agent: `opencode run --agent "sin-devicecontrol-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-devicecontrol-agent.log`
...
1. Collect logs: `journalctl -u sin-devicecontrol-agent --no-pager > /tmp/sin-devicecontrol-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-devicecontrol-agent/config.yaml > /tmp/sin-devicecontrol-agent-config.txt`
