# Sin Medium Agent — Troubleshooting Guide

> **Agent:** sin-medium-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-medium-agent`
2. Restart agent: `opencode run --agent "sin-medium-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-medium-agent.log`
...
1. Collect logs: `journalctl -u sin-medium-agent --no-pager > /tmp/sin-medium-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-medium-agent/config.yaml > /tmp/sin-medium-agent-config.txt`
