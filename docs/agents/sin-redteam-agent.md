# Sin Redteam Agent — Troubleshooting Guide

> **Agent:** sin-redteam-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-redteam-agent`
2. Restart agent: `opencode run --agent "sin-redteam-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-redteam-agent.log`
...
1. Collect logs: `journalctl -u sin-redteam-agent --no-pager > /tmp/sin-redteam-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-redteam-agent/config.yaml > /tmp/sin-redteam-agent-config.txt`
