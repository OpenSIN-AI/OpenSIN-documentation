# Sin Notes Agent — Troubleshooting Guide

> **Agent:** sin-notes-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-notes-agent`
2. Restart agent: `opencode run --agent "sin-notes-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-notes-agent.log`
...
1. Collect logs: `journalctl -u sin-notes-agent --no-pager > /tmp/sin-notes-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-notes-agent/config.yaml > /tmp/sin-notes-agent-config.txt`
