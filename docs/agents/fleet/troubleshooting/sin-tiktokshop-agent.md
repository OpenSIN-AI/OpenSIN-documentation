# Sin Tiktokshop Agent — Troubleshooting Guide

> **Agent:** sin-tiktokshop-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-tiktokshop-agent`
2. Restart agent: `opencode run --agent "sin-tiktokshop-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-tiktokshop-agent.log`
...
1. Collect logs: `journalctl -u sin-tiktokshop-agent --no-pager > /tmp/sin-tiktokshop-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-tiktokshop-agent/config.yaml > /tmp/sin-tiktokshop-agent-config.txt`
