# Sin Terms Agent — Troubleshooting Guide

> **Agent:** sin-terms-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-terms-agent`
2. Restart agent: `opencode run --agent "sin-terms-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-terms-agent.log`
...
1. Collect logs: `journalctl -u sin-terms-agent --no-pager > /tmp/sin-terms-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-terms-agent/config.yaml > /tmp/sin-terms-agent-config.txt`
