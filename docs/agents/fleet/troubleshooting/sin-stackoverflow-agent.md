# Sin Stackoverflow Agent — Troubleshooting Guide

> **Agent:** sin-stackoverflow-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-stackoverflow-agent`
2. Restart agent: `opencode run --agent "sin-stackoverflow-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-stackoverflow-agent.log`
...
1. Collect logs: `journalctl -u sin-stackoverflow-agent --no-pager > /tmp/sin-stackoverflow-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-stackoverflow-agent/config.yaml > /tmp/sin-stackoverflow-agent-config.txt`
