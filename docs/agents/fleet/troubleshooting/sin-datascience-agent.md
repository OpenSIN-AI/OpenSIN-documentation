# Sin Datascience Agent — Troubleshooting Guide

> **Agent:** sin-datascience-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-datascience-agent`
2. Restart agent: `opencode run --agent "sin-datascience-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-datascience-agent.log`
...
1. Collect logs: `journalctl -u sin-datascience-agent --no-pager > /tmp/sin-datascience-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-datascience-agent/config.yaml > /tmp/sin-datascience-agent-config.txt`
