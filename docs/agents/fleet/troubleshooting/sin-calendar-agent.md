# Sin Calendar Agent — Troubleshooting Guide

> **Agent:** sin-calendar-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-calendar-agent`
2. Restart agent: `opencode run --agent "sin-calendar-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-calendar-agent.log`
...
1. Collect logs: `journalctl -u sin-calendar-agent --no-pager > /tmp/sin-calendar-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-calendar-agent/config.yaml > /tmp/sin-calendar-agent-config.txt`
