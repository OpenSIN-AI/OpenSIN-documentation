# Sin Qa Agent — Troubleshooting Guide

> **Agent:** sin-qa-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-qa-agent`
2. Restart agent: `opencode run --agent "sin-qa-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-qa-agent.log`
...
1. Collect logs: `journalctl -u sin-qa-agent --no-pager > /tmp/sin-qa-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-qa-agent/config.yaml > /tmp/sin-qa-agent-config.txt`
