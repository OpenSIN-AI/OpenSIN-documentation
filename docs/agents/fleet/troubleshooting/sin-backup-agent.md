# Sin Backup Agent — Troubleshooting Guide

> **Agent:** sin-backup-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-backup-agent`
2. Restart agent: `opencode run --agent "sin-backup-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-backup-agent.log`
...
1. Collect logs: `journalctl -u sin-backup-agent --no-pager > /tmp/sin-backup-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-backup-agent/config.yaml > /tmp/sin-backup-agent-config.txt`
