# Sin Monitoring Agent — Troubleshooting Guide

> **Agent:** sin-monitoring-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-monitoring-agent`
2. Restart agent: `opencode run --agent "sin-monitoring-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-monitoring-agent.log`
...
1. Collect logs: `journalctl -u sin-monitoring-agent --no-pager > /tmp/sin-monitoring-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-monitoring-agent/config.yaml > /tmp/sin-monitoring-agent-config.txt`
