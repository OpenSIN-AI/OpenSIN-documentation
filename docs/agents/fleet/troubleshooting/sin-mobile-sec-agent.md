# Sin Mobile Sec Agent — Troubleshooting Guide

> **Agent:** sin-mobile-sec-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-mobile-sec-agent`
2. Restart agent: `opencode run --agent "sin-mobile-sec-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-mobile-sec-agent.log`
...
1. Collect logs: `journalctl -u sin-mobile-sec-agent --no-pager > /tmp/sin-mobile-sec-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-mobile-sec-agent/config.yaml > /tmp/sin-mobile-sec-agent-config.txt`
