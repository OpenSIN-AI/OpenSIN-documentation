# Sin Dns Agent — Troubleshooting Guide

> **Agent:** sin-dns-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-dns-agent`
2. Restart agent: `opencode run --agent "sin-dns-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-dns-agent.log`
...
1. Collect logs: `journalctl -u sin-dns-agent --no-pager > /tmp/sin-dns-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-dns-agent/config.yaml > /tmp/sin-dns-agent-config.txt`
