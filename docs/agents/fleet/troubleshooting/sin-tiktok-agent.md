# Sin Tiktok Agent — Troubleshooting Guide

> **Agent:** sin-tiktok-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-tiktok-agent`
2. Restart agent: `opencode run --agent "sin-tiktok-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-tiktok-agent.log`
...
1. Collect logs: `journalctl -u sin-tiktok-agent --no-pager > /tmp/sin-tiktok-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-tiktok-agent/config.yaml > /tmp/sin-tiktok-agent-config.txt`
