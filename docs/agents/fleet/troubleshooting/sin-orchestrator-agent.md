# Sin Orchestrator Agent — Troubleshooting Guide

> **Agent:** sin-orchestrator-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-orchestrator-agent`
2. Restart agent: `opencode run --agent "sin-orchestrator-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-orchestrator-agent.log`
...
1. Collect logs: `journalctl -u sin-orchestrator-agent --no-pager > /tmp/sin-orchestrator-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-orchestrator-agent/config.yaml > /tmp/sin-orchestrator-agent-config.txt`
