# Sin Photos Agent — Troubleshooting Guide

> **Agent:** sin-photos-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-photos-agent`
2. Restart agent: `opencode run --agent "sin-photos-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-photos-agent.log`
...
1. Collect logs: `journalctl -u sin-photos-agent --no-pager > /tmp/sin-photos-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-photos-agent/config.yaml > /tmp/sin-photos-agent-config.txt`
