# Sin Producthunt Agent — Troubleshooting Guide

> **Agent:** sin-producthunt-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-producthunt-agent`
2. Restart agent: `opencode run --agent "sin-producthunt-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-producthunt-agent.log`
...
1. Collect logs: `journalctl -u sin-producthunt-agent --no-pager > /tmp/sin-producthunt-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-producthunt-agent/config.yaml > /tmp/sin-producthunt-agent-config.txt`
