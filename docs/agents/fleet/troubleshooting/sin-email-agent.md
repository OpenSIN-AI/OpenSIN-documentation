# Sin Email Agent — Troubleshooting Guide

> **Agent:** sin-email-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-email-agent`
2. Restart agent: `opencode run --agent "sin-email-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-email-agent.log`
...
1. Collect logs: `journalctl -u sin-email-agent --no-pager > /tmp/sin-email-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-email-agent/config.yaml > /tmp/sin-email-agent-config.txt`
