# Sin Facetime Agent — Troubleshooting Guide

> **Agent:** sin-facetime-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-facetime-agent`
2. Restart agent: `opencode run --agent "sin-facetime-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-facetime-agent.log`
...
1. Collect logs: `journalctl -u sin-facetime-agent --no-pager > /tmp/sin-facetime-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-facetime-agent/config.yaml > /tmp/sin-facetime-agent-config.txt`
