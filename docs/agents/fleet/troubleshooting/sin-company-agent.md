# Sin Company Agent — Troubleshooting Guide

> **Agent:** sin-company-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-company-agent`
2. Restart agent: `opencode run --agent "sin-company-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-company-agent.log`
...
1. Collect logs: `journalctl -u sin-company-agent --no-pager > /tmp/sin-company-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-company-agent/config.yaml > /tmp/sin-company-agent-config.txt`
