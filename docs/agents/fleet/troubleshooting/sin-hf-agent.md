# Sin Hf Agent — Troubleshooting Guide

> **Agent:** sin-hf-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-hf-agent`
2. Restart agent: `opencode run --agent "sin-hf-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-hf-agent.log`
...
1. Collect logs: `journalctl -u sin-hf-agent --no-pager > /tmp/sin-hf-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-hf-agent/config.yaml > /tmp/sin-hf-agent-config.txt`
