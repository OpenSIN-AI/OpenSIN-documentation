# Sin Safari Agent — Troubleshooting Guide

> **Agent:** sin-safari-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-safari-agent`
2. Restart agent: `opencode run --agent "sin-safari-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-safari-agent.log`
...
1. Collect logs: `journalctl -u sin-safari-agent --no-pager > /tmp/sin-safari-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-safari-agent/config.yaml > /tmp/sin-safari-agent-config.txt`
