# Sin Contract Agent — Troubleshooting Guide

> **Agent:** sin-contract-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-contract-agent`
2. Restart agent: `opencode run --agent "sin-contract-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-contract-agent.log`
...
1. Collect logs: `journalctl -u sin-contract-agent --no-pager > /tmp/sin-contract-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-contract-agent/config.yaml > /tmp/sin-contract-agent-config.txt`
