# Sin Cloudflare Agent — Troubleshooting Guide

> **Agent:** sin-cloudflare-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-cloudflare-agent`
2. Restart agent: `opencode run --agent "sin-cloudflare-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-cloudflare-agent.log`
...
1. Collect logs: `journalctl -u sin-cloudflare-agent --no-pager > /tmp/sin-cloudflare-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-cloudflare-agent/config.yaml > /tmp/sin-cloudflare-agent-config.txt`
