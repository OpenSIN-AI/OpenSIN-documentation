# Sin Security Agent — Troubleshooting Guide

> **Agent:** sin-security-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-security-agent`
2. Restart agent: `opencode run --agent "sin-security-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-security-agent.log`
...
1. Collect logs: `journalctl -u sin-security-agent --no-pager > /tmp/sin-security-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-security-agent/config.yaml > /tmp/sin-security-agent-config.txt`
