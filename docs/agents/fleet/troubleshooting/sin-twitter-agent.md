# Sin Twitter Agent — Troubleshooting Guide

> **Agent:** sin-twitter-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-twitter-agent`
2. Restart agent: `opencode run --agent "sin-twitter-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-twitter-agent.log`
...
1. Collect logs: `journalctl -u sin-twitter-agent --no-pager > /tmp/sin-twitter-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-twitter-agent/config.yaml > /tmp/sin-twitter-agent-config.txt`
