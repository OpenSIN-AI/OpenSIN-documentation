# Sin Crypto Agent — Configuration Reference

> **Agent:** sin-crypto-agent | **Team:** Crypto | **Status:** ✅ Active

## Agent Card

```json
{
  "name": "sin-crypto-agent",
  "version": "1.0.0",
  "description": "Autonomous A2A agent for sin crypto agent",
  "team": "crypto",
...
|----------|----------|---------|-------------|
| `SIN_CRYPTO_AGENT_API_KEY` | No | — | API key for external services |
| `SIN_CRYPTO_AGENT_ENDPOINT` | No | — | Custom endpoint URL |
| `SIN_CRYPTO_AGENT_TIMEOUT` | No | 30 | Request timeout in seconds |
| `SIN_CRYPTO_AGENT_RETRIES` | No | 3 | Number of retries on failure |
| `SIN_CRYPTO_AGENT_LOG_LEVEL` | No | info | Logging level |
