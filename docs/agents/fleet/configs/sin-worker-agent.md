# Sin Worker Agent — Configuration Reference

> **Agent:** sin-worker-agent | **Team:** Worker | **Status:** ✅ Active

## Agent Card

```json
{
  "name": "sin-worker-agent",
  "version": "1.0.0",
  "description": "Autonomous A2A agent for sin worker agent",
  "team": "worker",
...
|----------|----------|---------|-------------|
| `SIN_WORKER_AGENT_API_KEY` | No | — | API key for external services |
| `SIN_WORKER_AGENT_ENDPOINT` | No | — | Custom endpoint URL |
| `SIN_WORKER_AGENT_TIMEOUT` | No | 30 | Request timeout in seconds |
| `SIN_WORKER_AGENT_RETRIES` | No | 3 | Number of retries on failure |
| `SIN_WORKER_AGENT_LOG_LEVEL` | No | info | Logging level |
