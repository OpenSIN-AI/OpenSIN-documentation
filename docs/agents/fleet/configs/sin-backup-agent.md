# Sin Backup Agent — Configuration Reference

> **Agent:** sin-backup-agent | **Team:** Backup | **Status:** ✅ Active

## Agent Card

```json
{
  "name": "sin-backup-agent",
  "version": "1.0.0",
  "description": "Autonomous A2A agent for sin backup agent",
  "team": "backup",
...
|----------|----------|---------|-------------|
| `SIN_BACKUP_AGENT_API_KEY` | No | — | API key for external services |
| `SIN_BACKUP_AGENT_ENDPOINT` | No | — | Custom endpoint URL |
| `SIN_BACKUP_AGENT_TIMEOUT` | No | 30 | Request timeout in seconds |
| `SIN_BACKUP_AGENT_RETRIES` | No | 3 | Number of retries on failure |
| `SIN_BACKUP_AGENT_LOG_LEVEL` | No | info | Logging level |
