# Sin Devops Agent — Troubleshooting Guide

> **Agent:** sin-devops-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-devops-agent`
2. Restart agent: `opencode run --agent "sin-devops-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-devops-agent.log`
...
1. Collect logs: `journalctl -u sin-devops-agent --no-pager > /tmp/sin-devops-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-devops-agent/config.yaml > /tmp/sin-devops-agent-config.txt`
