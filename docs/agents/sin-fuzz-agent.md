# Sin Fuzz Agent — Troubleshooting Guide

> **Agent:** sin-fuzz-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-fuzz-agent`
2. Restart agent: `opencode run --agent "sin-fuzz-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-fuzz-agent.log`
...
1. Collect logs: `journalctl -u sin-fuzz-agent --no-pager > /tmp/sin-fuzz-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-fuzz-agent/config.yaml > /tmp/sin-fuzz-agent-config.txt`
