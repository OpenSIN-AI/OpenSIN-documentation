# Sin Oci Agent — Troubleshooting Guide

> **Agent:** sin-oci-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-oci-agent`
2. Restart agent: `opencode run --agent "sin-oci-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-oci-agent.log`
...
1. Collect logs: `journalctl -u sin-oci-agent --no-pager > /tmp/sin-oci-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-oci-agent/config.yaml > /tmp/sin-oci-agent-config.txt`
