# Sin Instagram Agent — Troubleshooting Guide

> **Agent:** sin-instagram-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-instagram-agent`
2. Restart agent: `opencode run --agent "sin-instagram-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-instagram-agent.log`
...
1. Collect logs: `journalctl -u sin-instagram-agent --no-pager > /tmp/sin-instagram-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-instagram-agent/config.yaml > /tmp/sin-instagram-agent-config.txt`
