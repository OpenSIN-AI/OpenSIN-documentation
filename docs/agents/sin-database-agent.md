# Sin Database Agent — Troubleshooting Guide

> **Agent:** sin-database-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-database-agent`
2. Restart agent: `opencode run --agent "sin-database-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-database-agent.log`
...
1. Collect logs: `journalctl -u sin-database-agent --no-pager > /tmp/sin-database-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-database-agent/config.yaml > /tmp/sin-database-agent-config.txt`
