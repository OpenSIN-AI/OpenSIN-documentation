# Sin Logistic Agent — Troubleshooting Guide

> **Agent:** sin-logistic-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-logistic-agent`
2. Restart agent: `opencode run --agent "sin-logistic-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-logistic-agent.log`
...
1. Collect logs: `journalctl -u sin-logistic-agent --no-pager > /tmp/sin-logistic-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-logistic-agent/config.yaml > /tmp/sin-logistic-agent-config.txt`
