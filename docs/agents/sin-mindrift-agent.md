# Sin Mindrift Agent — Troubleshooting Guide

> **Agent:** sin-mindrift-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-mindrift-agent`
2. Restart agent: `opencode run --agent "sin-mindrift-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-mindrift-agent.log`
...
1. Collect logs: `journalctl -u sin-mindrift-agent --no-pager > /tmp/sin-mindrift-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-mindrift-agent/config.yaml > /tmp/sin-mindrift-agent-config.txt`
