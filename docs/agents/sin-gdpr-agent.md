# Sin Gdpr Agent — Troubleshooting Guide

> **Agent:** sin-gdpr-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-gdpr-agent`
2. Restart agent: `opencode run --agent "sin-gdpr-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-gdpr-agent.log`
...
1. Collect logs: `journalctl -u sin-gdpr-agent --no-pager > /tmp/sin-gdpr-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-gdpr-agent/config.yaml > /tmp/sin-gdpr-agent-config.txt`
