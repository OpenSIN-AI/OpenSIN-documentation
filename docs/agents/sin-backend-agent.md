# Sin Backend Agent — Troubleshooting Guide

> **Agent:** sin-backend-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-backend-agent`
2. Restart agent: `opencode run --agent "sin-backend-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-backend-agent.log`
...
1. Collect logs: `journalctl -u sin-backend-agent --no-pager > /tmp/sin-backend-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-backend-agent/config.yaml > /tmp/sin-backend-agent-config.txt`
