# Sin Privacy Agent — Troubleshooting Guide

> **Agent:** sin-privacy-agent | **Category:** Troubleshooting | **Status:** ✅ Active
...
**Resolution:**
1. Check agent status: `sin-n8n get sin-privacy-agent`
2. Restart agent: `opencode run --agent "sin-privacy-agent" "ping"`
3. Check logs: `tail -f /var/log/opensin/sin-privacy-agent.log`
...
1. Collect logs: `journalctl -u sin-privacy-agent --no-pager > /tmp/sin-privacy-agent-logs.txt`
2. Collect config: `cat /etc/opensin/sin-privacy-agent/config.yaml > /tmp/sin-privacy-agent-config.txt`
